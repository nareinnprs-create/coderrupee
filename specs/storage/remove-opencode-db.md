# Remove `packages/coderrupee/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/coderrupee/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the coderrupee legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/coderrupee/src/storage/db.ts` are concentrated in 22 source files:

- `packages/coderrupee/src/account/repo.ts`
- `packages/coderrupee/src/cli/cmd/db.ts`
- `packages/coderrupee/src/cli/cmd/import.ts`
- `packages/coderrupee/src/cli/cmd/stats.ts`
- `packages/coderrupee/src/control-plane/workspace.ts`
- `packages/coderrupee/src/index.ts`
- `packages/coderrupee/src/node.ts`
- `packages/coderrupee/src/permission/index.ts`
- `packages/coderrupee/src/project/project.ts`
- `packages/coderrupee/src/server/projectors.ts`
- `packages/coderrupee/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/coderrupee/src/server/shared/fence.ts`
- `packages/coderrupee/src/session/message-v2.ts`
- `packages/coderrupee/src/session/projectors.ts`
- `packages/coderrupee/src/session/prompt.ts`
- `packages/coderrupee/src/session/session.ts`
- `packages/coderrupee/src/session/todo.ts`
- `packages/coderrupee/src/share/share-next.ts`
- `packages/coderrupee/src/storage/db.ts`
- `packages/coderrupee/src/sync/index.ts`
- `packages/coderrupee/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy coderrupee database wrapper; `packages/coderrupee/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the coderrupee wrapper.
...
Status: Completed. `SyncEvent` and the coderrupee projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy coderrupee database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy coderrupee database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy coderrupee database wrapper.
...
All migration groups are complete or superseded. `packages/coderrupee/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No coderrupee data-migration group remains.

The previous coderrupee `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate coderrupee data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/coderrupee`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/coderrupee/src`
- `bun typecheck` from `packages/coderrupee`
- Relevant package tests from `packages/coderrupee`, not the repo root
