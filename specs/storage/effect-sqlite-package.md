...
Create a small workspace package that vendors the Drizzle `effect-sqlite` adapter shape for our repo. This is not an coderrupee storage abstraction. It is a local package that ports the Drizzle Effect SQLite implementation so we can use it before/independently of upstream release timing.

`packages/coderrupee` will use it internally, but the package itself should be generic: Drizzle + Effect + SQLite. No coderrupee paths, migrations, tables, transaction hooks, post-commit behavior, or domain language should live in this package.
...
- `@coderrupee/effect-drizzle-sqlite`
...
- Opencode-specific path/channel selection stays in `packages/coderrupee`.

## Opencode Adoption Notes

These are not package requirements, but they matter for the later coderrupee adoption PR.

The current `packages/coderrupee/src/storage/db.ts` has two non-obvious semantics that the coderrupee wrapper must preserve when it consumes this adapter:
...
The coderrupee wrapper can implement that using Effect context instead of `LocalContext`:
...
Do not remove this behavior while moving coderrupee to Effect SQLite. `SyncEvent.run` depends on transaction composability and `behavior: "immediate"` for sequencing correctness.

## Migration Strategy

1. Add `@coderrupee/effect-drizzle-sqlite` with a minimal in-memory/file SQLite test schema.
...
4. Add `@coderrupee/effect-drizzle-sqlite` as a dependency of `packages/coderrupee`.
5. Port `packages/coderrupee/src/storage/db.ts` to be a thin compatibility wrapper over the adapter plus coderrupee-specific transaction/post-commit context.
...
8. Only then build domain stores like session/message/project stores on top of coderrupee's storage wrapper.
...
An Effect Drizzle SQLite package lets us vendor the adapter once. Then coderrupee can build its own storage wrapper on top, and `SessionStorage`, `MessageStorage`, event store, and projector writes can all share the same transaction and migration model.
...
- Should `afterCommit` stay coderrupee-specific until event publishing moves? Default answer: yes.
...
- Do CLI/admin raw SQL and sqlite shell stay in `packages/coderrupee`, or does the storage package expose backend capabilities for them?
...
- Use a tiny test schema, not coderrupee domain tables.
- Prove Effect Drizzle SQLite queries, transactions, and migrations.
- Do not migrate `packages/coderrupee` yet except possibly adding the dependency if needed for typechecking.

That gives us a focused place to validate the Effect SQLite approach before disturbing coderrupee's current database runtime.
