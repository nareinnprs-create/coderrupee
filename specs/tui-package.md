...
Move the canonical CoderRupee terminal application from
`packages/coderrupee/src/cli/cmd/tui` into a self-contained workspace package while
...
generic presentation libraries. It must not depend on `packages/coderrupee`,
`packages/cli`, or `@coderrupee/core`.

The SDK is the TUI's CoderRupee boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/coderrupee`.
- Do not replace private `packages/coderrupee` imports with `@coderrupee/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@coderrupee/tui`.
...
- CoderRupee domain data displayed by the TUI
...
```text
packages/coderrupee/src/cli/cmd/tui
```

Its private dependency on `packages/coderrupee` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/coderrupee/src/*`.
...
Status: Completed. The private `@coderrupee/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@coderrupee/tui`.
...
- The package has no dependency on `coderrupee`, `@coderrupee/cli`, or
  `@coderrupee/core`.
...
primitives now live in `@coderrupee/tui`. The legacy host consumes explicit
...
and build inputs through `@coderrupee/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads CoderRupee globals or process state;
...
Replace process-global CoderRupee state with resolved TUI inputs.
...
`@coderrupee/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/coderrupee` initially.
...
canonical in `@coderrupee/tui`. Configured references resolve through the new
...
adapters rather than CoderRupee domain implementations.

Make the SDK the only CoderRupee domain boundary used by the TUI.
...
- Domain-facing TUI code imports CoderRupee data and operations only from
  `@coderrupee/sdk`.
...
`@coderrupee/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/coderrupee`.
...
- The package has no imports from `packages/coderrupee`, `packages/cli`, or
  `@coderrupee/core`.
...
public `@coderrupee/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/coderrupee`.
- Keep the legacy embedded worker and server startup in `packages/coderrupee`.
- Change those adapters to load config, create transport inputs, and call the
  public `@coderrupee/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/coderrupee`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@coderrupee/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@coderrupee/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for CoderRupee domain state.
...
cd packages/coderrupee && bun typecheck
...
rg '@coderrupee/core|packages/coderrupee|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/coderrupee/src packages/coderrupee/test
```
...
- Build the current-platform `packages/coderrupee` binary.
