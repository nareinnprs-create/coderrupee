...
The coderrupee server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@coderrupee/llm` tool loop after replacing its
...
- plugins get global "coderrupee" instance like in that post i showed
- coderrupee instance has stuff like `coderrupee.session.prompt()` or
  `coderrupee.tool.register({...})`
...
- decide whether to preserve deprecated `@coderrupee/llm` orchestration exports
