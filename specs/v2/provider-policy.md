...
Both `action` and `resource` use coderrupee's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.coderrupee` files is intentionally deferred until `.coderrupee` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether coderrupee permits an operation through its managed execution paths.
