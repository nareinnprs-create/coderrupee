...
import { createOpencode } from "@coderrupee/sdk"
...
`)
  ...
  const coderrupee = await createOpencode({ port: 0 })
  ...
  try {
    ...
    const session = await coderrupee.client.session.create()
    const result = await coderrupee.client.session
      .prompt({
