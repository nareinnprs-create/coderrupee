import { Message, Model, Part, Session, SessionStatus, SnapshotFileDiff, UserMessage } from "@coderrupee/sdk/v2"
import { SessionTurn } from "@coderrupee/session-ui/session-turn"
import { SessionReview } from "@coderrupee/session-ui/session-review"
import { DataProvider } from "@coderrupee/session-ui/context"
import { FileComponentProvider } from "@coderrupee/ui/context/file"
import { WorkerPoolProvider } from "@coderrupee/ui/context/worker-pool"
import { createAsync, query, useParams } from "@solidjs/router"
import { createMemo, createSignal, ErrorBoundary, For, Match, Show, Switch } from "solid-js"
import { Share } from "~/core/share"
import { Logo, Mark } from "@coderrupee/ui/logo"
import { IconButton } from "@coderrupee/ui/icon-button"
import { ProviderIcon } from "@coderrupee/ui/provider-icon"
import { iife } from "@coderrupee/core/util/iife"
import { Binary } from "@coderrupee/core/util/binary"
import { NamedError } from "@coderrupee/core/util/error"
import { DateTime } from "luxon"
import { createStore } from "solid-js/store"
import NotFound from "../[...404]"
import { Tabs } from "@coderrupee/ui/tabs"
import { MessageNav } from "@coderrupee/session-ui/message-nav"
import { FileSSR } from "@coderrupee/session-ui/file-ssr"
...
const ClientOnlyWorkerPoolProvider = clientOnly(() =>
  import("@coderrupee/session-ui/pierre/worker").then((m) => ({
      ...
      <Show when={data()}>
        {(data) => {
          ...
          const ogImage = createMemo(() => {
            ...
            return `https://social-cards.sst.dev/coderrupee-share/${encodedTitle}.png?model=${modelParam}&version=${version}&id=${data().shareID}`
          })

          return (
            <>
              <Show when={info().title}>
                <Title>{info().title} | CoderRupee</Title>
              </Show>
              <Meta name="description" content="coderrupee - The AI coding agent built for the terminal." />
                        ...
                        <div class="relative bg-background-stronger w-screen h-screen overflow-hidden flex flex-col">
                          <header class="h-12 px-6 py-2 flex items-center justify-between self-stretch bg-background-base border-b border-border-weak-base">
                            ...
                            <div class="flex gap-3 items-center">
                              <IconButton
                                as={"a"}
                                href="https://github.com/coderrupee/coderrupee"
                                target="_blank"
                                icon="github"
                                variant="ghost"
                              />
                              <IconButton
                                as={"a"}
                                href="https://coderrupee.ai/discord"
                                target="_blank"
                                icon="discord"
                                variant="ghost"
                              />
