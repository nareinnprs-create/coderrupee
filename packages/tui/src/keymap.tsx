...
export const CODERRUPEE_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const CODERRUPEE_MODE_KEY = "coderrupee.mode"
...
export function createOpencodeModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(CODERRUPEE_MODE_KEY, CODERRUPEE_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(CODERRUPEE_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(CODERRUPEE_MODE_KEY, stack.at(-1)?.mode ?? CODERRUPEE_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? CODERRUPEE_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(CODERRUPEE_MODE_KEY, undefined)
