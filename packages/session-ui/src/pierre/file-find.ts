...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("coderrupee-find")
  api.delete("coderrupee-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("coderrupee-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("coderrupee-find", new Highlight(...rest))
    return true
  }
