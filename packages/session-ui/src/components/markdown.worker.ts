...
import { createMarkdownParser } from "@coderrupee/ui/context/marked-parser"
import { CoderRupeeTheme } from "@coderrupee/ui/context/marked-theme"
import {
  bundledLanguages,
  createHighlighter,
  getTokenStyleObject,
...
const parser = createMarkdownParser(async (code, language) => {
  ...
  return instance.codeToHtml(code, { lang: name as BundledLanguage, theme: "CoderRupee", tabindex: false })
})
...
async function highlight(request: Extract<MarkdownWorkerRequest, { type: "highlight" }>) {
  try {
    ...
    if (request.complete) {
      const result = instance.codeToTokens(request.text, { lang: language as BundledLanguage, theme: "CoderRupee" })
    ...
    const stream = reset
      ? {
          language,
          source: "",
          tokenizer: new ShikiStreamTokenizer({ highlighter: instance, lang: language, theme: "CoderRupee" }),
        }
      : previous
...
function getHighlighter() {
  return (highlighter ??= createHighlighter({ themes: [CoderRupeeTheme], langs: [] }))
}
