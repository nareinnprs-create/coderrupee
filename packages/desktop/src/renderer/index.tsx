...
import type { UpdaterState } from "@coderrupee/app/updater"
...
import { useTheme } from "@coderrupee/ui/theme/context"
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.CODERRUPEE_CHANNEL === "prod" &&
...
const deepLinkEvent = "coderrupee:deep-link"
...
const emitDeepLinks = (urls: string[]) => {
  ...
  window.__CODERRUPEE__ ??= {}
  const pending = window.__CODERRUPEE__.deepLinks ?? []
  window.__CODERRUPEE__.deepLinks = [...pending, ...urls]
...
function windowLastActiveUrlKey(windowID: string) {
  return `coderrupee.desktop.window.${windowID}.last-active-url`
}
  ...
  return {
    ...
    notify: async (title, description, onClick) => {
      ...
      const notification = new Notification(title, {
        body: description ?? "",
        icon: "https://coderrupee.ai/favicon-96x96-v3.png",
      })
...
function DesktopRoot(props: { windowState: DesktopWindowState }) {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("coderrupee.global.dat").getItem("language")
