...
export async function spawnWslSidecar(
  ...
  const coderrupee = await resolveWslOpencode(distro)
  if (!coderrupee) throw new Error(nativeT("desktop.wsl.error.coderrupeeNotInstalled", { distro }))

  const port = await allocatePort()
  const password = randomUUID()
  const username = "coderrupee"
  const script = [
    "set -euo pipefail",
    'cd "$HOME" || cd /',
    ...
    "export PATH",
    "export WSLENV=",
    "export CODERRUPEE_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export CODERRUPEE_CLIENT=desktop",
    `export CODERRUPEE_SERVER_USERNAME=${shellEscape(username)}`,
    `export CODERRUPEE_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(coderrupee)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
