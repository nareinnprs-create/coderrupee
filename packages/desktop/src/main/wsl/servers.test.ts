...
test("rejects an update that did not install the desktop version", () => {
  ...
  expect(() => expectOpencodeVersion("1.14.35", "1.16.2")).toThrow(
    "CoderRupee update finished but Debian still reports 1.14.35; expected 1.16.2",
  )
})

test("restarts an existing distro server after updating CoderRupee", () => {
  expect(
    wslServerIdToRestart(
      [
        {
...
test("clears cached distro probes when removing a WSL server", () => {
  expect(
    clearWslDistroState(
      { Debian: { name: "Debian", canExecute: true, hasBash: true, hasCurl: true, error: null } },
      {
        Debian: {
          distro: "Debian",
          resolvedPath: "/home/luke/.coderrupee/bin/coderrupee",
          version: "1.16.2",
  ...
  ).toEqual({ distroProbes: {}, coderrupeeChecks: {} })
})
...
test("ignores stale background CoderRupee checks after removing a WSL server", async () => {
  persistedServers = []
  ...
  expect(controller.getState().coderrupeeChecks).toEqual({})
})

test("ignores stale startup CoderRupee checks after removing a WSL server", async () => {
...
test("probes addable distros in parallel before checking CoderRupee", async () => {
  persistedServers = []
  const started: string[] = []
  const release = new Map<string, () => void>()
  const coderrupee: string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...
    resolveOpencode: async (distro) => {
      coderrupee.push(distro)
      return "/home/me/.coderrupee/bin/coderrupee"
    },
  })
  ...
  expect(coderrupee).toEqual([])
  release.get("Debian")?.()
  release.get("Ubuntu")?.()
  await task

  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(coderrupee).toEqual(["Debian", "Ubuntu"])
  expect(Object.keys(controller.getState().coderrupeeChecks)).toEqual(["Debian", "Ubuntu"])
})

test("does not check CoderRupee in addable distros that cannot execute commands", async () => {
  persistedServers = []
  const coderrupee: string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...testControllerOptions(),
  ...
  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(coderrupee).toEqual(["Debian"])
  expect(Object.keys(controller.getState().coderrupeeChecks)).toEqual(["Debian"])
})
