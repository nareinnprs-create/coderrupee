...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.coderrupee.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `coderrupee.settings.dat` is special: it maps to the `coderrupee.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"coderrupee.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // coderrupee.settings.dat → the electron settings store ("coderrupee.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "coderrupee.global.dat").
  const storeName = filename === "coderrupee.settings.dat" ? "coderrupee.settings" : filename
