---
title: Config
description: Using the CoderRupee JSON config.
---

You can configure CoderRupee using a JSON config file.

---

## Format

CoderRupee supports both **JSON** and **JSONC** (JSON with Comments) formats.

```jsonc title="coderrupee.jsonc"
{
...
1. **Remote config** (from `.well-known/coderrupee`) - organizational defaults
2. **Global config** (`~/.config/coderrupee/coderrupee.json`) - user preferences
3. **Custom config** (`CODERRUPEE_CONFIG` env var) - custom overrides
4. **Project config** (`coderrupee.json` in project) - project-specific settings
5. **`.coderrupee` directories** - agents, commands, plugins
6. **Inline config** (`CODERRUPEE_CONFIG_CONTENT` env var) - runtime overrides
7. **Managed config files** (`/Library/Application Support/coderrupee/` on macOS) - admin-controlled
...
:::note
The `.coderrupee` and `~/.config/coderrupee` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Organizations can provide default configuration via the `.well-known/coderrupee` endpoint. This is fetched automatically when you authenticate with a provider that supports it.
...
```json title="Remote config from .well-known/coderrupee"
{
  "mcp": {
    "jira": {
      "type": "remote",
...
Place your global CoderRupee config in `~/.config/coderrupee/coderrupee.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/coderrupee/tui.json`.
...
Add `coderrupee.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When CoderRupee starts up, it first looks for a config file in the current directory, then traverses up to the nearest Git directory.
...
Specify a custom config file path using the `CODERRUPEE_CONFIG` environment variable.

```bash
export CODERRUPEE_CONFIG=/path/to/my/custom-config.json
coderrupee run "Hello world"
```
...
Specify a custom config directory using the `CODERRUPEE_CONFIG_DIR`
environment variable. This directory will be searched for agents, commands,
modes, and plugins just like the standard `.coderrupee` directory, and should
follow the same structure.

```bash
export CODERRUPEE_CONFIG_DIR=/path/to/my/config-directory
coderrupee run "Hello world"
```

The custom directory is loaded after the global config and `.coderrupee` directories, so it **can override** their settings.
...
Drop an `coderrupee.json` or `coderrupee.jsonc` file in the system managed config directory:

| Platform | Path                                     |
| -------- | ---------------------------------------- |
| macOS    | `/Library/Application Support/coderrupee/` |
| Linux    | `/etc/coderrupee/`                         |
| Windows  | `%ProgramData%\coderrupee`                 |

These directories require admin/root access to write, so users cannot modify them.

#### macOS managed preferences

On macOS, CoderRupee reads managed preferences from the `ai.coderrupee.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

CoderRupee checks these paths:

1. `/Library/Managed Preferences/<user>/ai.coderrupee.managed.plist`
2. `/Library/Managed Preferences/ai.coderrupee.managed.plist`

The plist keys map directly to `coderrupee.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.coderrupee.managed` PayloadType. The CoderRupee config keys go directly in the payload dict:
...
<dict>
  ...
  <array>
    <dict>
      <key>PayloadType</key>
      <string>ai.coderrupee.managed</string>
      <key>PayloadIdentifier</key>
      <string>com.example.coderrupee.config</string>
      <key>PayloadUUID</key>
  ...
  <key>PayloadIdentifier</key>
  <string>com.example.coderrupee</string>
  <key>PayloadUUID</key>
  <string>GENERATE-YOUR-OWN-UUID</string>
  <key>PayloadVersion</key>
  <integer>1</integer>
...
The server/runtime config schema is defined in [**`coderrupee.ai/config.json`**](https://coderrupee.ai/config.json).

TUI config uses [**`coderrupee.ai/tui.json`**](https://coderrupee.ai/tui.json).
...
Use `CODERRUPEE_TUI_CONFIG` to point to a custom TUI config file.
...
Legacy `theme`, `keybinds`, and `tui` keys in `coderrupee.json` are deprecated and automatically migrated when possible.

---

### Server

You can configure server settings for the `coderrupee serve` and `coderrupee web` commands through the `server` option.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your CoderRupee server.
- `mdnsDomain` - Custom domain name for mDNS service. Defaults to `coderrupee.local`. Useful for running multiple instances on the same network.
...
If not specified, CoderRupee will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
You can configure the providers and models you want to use in your CoderRupee config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, CoderRupee tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
Use the `experimental.policies` option to allow or deny CoderRupee actions on configured resources. Currently, policies can control which providers CoderRupee may use.
...
CoderRupee normalizes image attachments before sending them to the model. By default, images are resized when they exceed `2000x2000` pixels or `5242880` base64 bytes.
...
If an image still cannot fit after resizing, CoderRupee omits oversized tool-result images or fails oversized user-provided images with an image size error.
...
You can also define agents using markdown files in `~/.config/coderrupee/agents/` or `.coderrupee/agents/`. [Learn more here](/docs/agents).
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, CoderRupee will fall back to `"build"` with a warning.

This setting applies across all interfaces: TUI, CLI (`coderrupee run`), desktop app, and GitHub Action.
...
You can also define commands using markdown files in `~/.config/coderrupee/commands/` or `.coderrupee/commands/`. [Learn more here](/docs/commands).
...
CoderRupee uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
CoderRupee will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
By default, coderrupee **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
[Plugins](/docs/plugins) extend CoderRupee with custom tools, hooks, and integrations.

Place plugin files in `.coderrupee/plugins/` or `~/.config/coderrupee/plugins/`. You can also load plugins from npm through the `plugin` option.

```json title="coderrupee.json"
{
  "$schema": "https://coderrupee.ai/config.json",
  "plugin": ["coderrupee-helicone-session", "@my-org/custom-plugin"]
}
```
...
This is useful when you want to restrict CoderRupee to only use specific providers rather than disabling them one by one.
