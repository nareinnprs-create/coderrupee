---
title: Server
description: Interact with coderrupee server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `coderrupee serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an coderrupee client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `coderrupee.local` |
...
Set `CODERRUPEE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `coderrupee`, or set `CODERRUPEE_SERVER_USERNAME` to override it. This applies to both `coderrupee serve` and `coderrupee web`.
...
When you run `coderrupee` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the coderrupee server to interact with coderrupee programmatically.
:::

This architecture lets coderrupee support multiple clients and allows you to interact with coderrupee programmatically.

You can run `coderrupee serve` to start a standalone server. If you have the
coderrupee TUI running, `coderrupee serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the CoderRupee [IDE](/docs/ide) plugins.
...
The coderrupee server exposes the following APIs.
