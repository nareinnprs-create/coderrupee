...
You can add external tools to CoderRupee using the _Model Context Protocol_, or MCP. CoderRupee supports both local and remote servers.
...
You can define MCP servers in your [CoderRupee Config](https://coderrupee.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/coderrupee` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
CoderRupee automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, CoderRupee will:
...
If the server requires authentication, CoderRupee will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `coderrupee mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, CoderRupee will store the tokens securely in `~/.local/share/coderrupee/mcp-auth.json`.
...
Your MCPs are available as tools in CoderRupee, alongside built-in tools. So you can manage them through the CoderRupee config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect CoderRupee to your Sentry account.
