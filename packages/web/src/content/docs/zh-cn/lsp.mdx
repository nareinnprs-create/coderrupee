---
title: LSP 服务器
description: CoderRupee 与你的 LSP 服务器集成。
---

CoderRupee 可以与语言服务器协议（LSP）服务器集成，将诊断信息作为 agent 的反馈。

---

## 内置支持

CoderRupee 内置了多种适用于主流语言的 LSP 服务器：
...
:::note
你可以将 `CODERRUPEE_DISABLE_LSP_DOWNLOAD` 环境变量设置为 `true` 来禁用 LSP 服务器的自动下载。
:::
...
你可以通过 coderrupee 配置文件中的 `lsp` 部分来启用并自定义 LSP 服务器。
