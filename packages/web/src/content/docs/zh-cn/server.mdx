---
title: 服务器
description: 通过 HTTP 与 coderrupee 服务器交互。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`coderrupee serve` 命令运行一个无界面的 HTTP 服务器，暴露一个 OpenAPI 端点供 coderrupee 客户端使用。
...
| `--mdns-domain` | mDNS 服务的自定义域名 | `coderrupee.local` |
| `--cors`        | 额外允许的浏览器来源  | `[]`             |
...
设置 `CODERRUPEE_SERVER_PASSWORD` 以使用 HTTP 基本认证保护服务器。用户名默认为 `coderrupee`，也可以设置 `CODERRUPEE_SERVER_USERNAME` 来覆盖它。这适用于 `coderrupee serve` 和 `coderrupee web`。
...
当你运行 `coderrupee` 时，它会启动一个 TUI 和一个服务器。TUI 是与服务器通信的客户端。服务器暴露一个 OpenAPI 3.1 规范端点。该端点也用于生成 [SDK](/docs/sdk)。

:::tip
使用 coderrupee 服务器以编程方式与 coderrupee 交互。
:::

这种架构让 coderrupee 支持多个客户端，并允许你以编程方式与 coderrupee 交互。

你可以运行 `coderrupee serve` 来启动一个独立的服务器。如果你已经在运行 coderrupee TUI，`coderrupee serve` 会启动一个新的服务器。
...
[`/tui`](#tui) 端点可用于通过服务器驱动 TUI。例如，你可以预填充或运行一个提示词。此方式被 CoderRupee [IDE](/docs/ide) 插件所使用。
