...
description: Cree herramientas que LLM pueda llamar en coderrupee.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de coderrupee como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.coderrupee/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/coderrupee/tools/`.
...
```ts title=".coderrupee/tools/database.ts" {1}
...
```ts title=".coderrupee/tools/math.ts"
import { tool } from "@coderrupee/plugin"
...
```ts title=".coderrupee/tools/bash.ts"
import { tool } from "@coderrupee/plugin"
...
```ts title=".coderrupee/tools/project.ts" {8}
import { tool } from "@coderrupee/plugin"
...
```python title=".coderrupee/tools/add.py"
import sys
...
```ts title=".coderrupee/tools/python-add.ts" {10}
import { tool } from "@coderrupee/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".coderrupee/tools/add.py")
