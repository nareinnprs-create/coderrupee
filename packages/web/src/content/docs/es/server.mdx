---
title: Servidor
description: Interactuar con el servidor coderrupee a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `coderrupee serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente coderrupee puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `coderrupee.local` |
...
Configure `CODERRUPEE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `coderrupee`, o configure `CODERRUPEE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `coderrupee serve` como a `coderrupee web`.
...
Cuando ejecuta `coderrupee`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor coderrupee para interactuar con coderrupee mediante programación.
:::

Esta arquitectura permite que coderrupee admita múltiples clientes y le permite interactuar con coderrupee mediante programación.

Puede ejecutar `coderrupee serve` para iniciar un servidor independiente. Si tienes el
coderrupee TUI ejecutándose, `coderrupee serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos CoderRupee [IDE](/docs/ide).
...
El servidor coderrupee expone las siguientes API.
