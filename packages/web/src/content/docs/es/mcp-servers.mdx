...
Puede agregar herramientas externas a CoderRupee utilizando el _Model Context Protocol_ o MCP. CoderRupee admite servidores locales y remotos.
...
Puede definir servidores MCP en su [CoderRupee Config](https://coderrupee.ai/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Las organizaciones pueden proporcionar servidores MCP predeterminados a través de su punto final `.well-known/coderrupee`. Estos servidores pueden estar deshabilitados de forma predeterminada, lo que permite a los usuarios optar por los que necesitan.
...
CoderRupee maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, CoderRupee:
...
Si el servidor requiere autenticación, CoderRupee le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `coderrupee mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, CoderRupee almacenará los tokens de forma segura en `~/.local/share/coderrupee/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en CoderRupee, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración CoderRupee como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar CoderRupee a su cuenta Sentry.
