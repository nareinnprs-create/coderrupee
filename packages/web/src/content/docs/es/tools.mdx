...
Las herramientas permiten que LLM realice acciones en su código base. CoderRupee viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en CoderRupee.
...
:::note
Esta herramienta solo está disponible cuando `CODERRUPEE_EXPERIMENTAL_LSP_TOOL=true` (o `CODERRUPEE_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor CoderRupee o cuando la variable de entorno `CODERRUPEE_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar CoderRupee:

```bash
CODERRUPEE_ENABLE_EXA=1 coderrupee
```
