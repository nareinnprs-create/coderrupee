---
title: Complementos
description: Escriba sus propios complementos para extender CoderRupee.
---

Los complementos le permiten extender CoderRupee al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de CoderRupee.
...
- `.coderrupee/plugins/` - Complementos a nivel de proyecto
- `~/.config/coderrupee/plugins/` - Complementos globales
...
```json title="coderrupee.json"
{
  ...
  "plugin": ["coderrupee-helicone-session", "coderrupee-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/coderrupee/node_modules/`.
...
1. Configuración global (`~/.config/coderrupee/coderrupee.json`)
2. Configuración del proyecto (`coderrupee.json`)
3. Directorio global de complementos (`~/.config/coderrupee/plugins/`)
4. Directorio de complementos del proyecto (`.coderrupee/plugins/`)
...
```json title=".coderrupee/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
CoderRupee ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".coderrupee/plugins/my-plugin.ts"
...
```js title=".coderrupee/plugins/example.js"
...
- `client`: Un cliente SDK coderrupee para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar coderrupee.
...
```js title=".coderrupee/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "coderrupee"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio CoderRupee, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que coderrupee lea archivos `.env`:

```javascript title=".coderrupee/plugins/env-protection.js"
...
```javascript title=".coderrupee/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a coderrupee:

```ts title=".coderrupee/plugins/custom-tools.ts"
import { type Plugin, tool } from "@coderrupee/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que coderrupee puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para coderrupee junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://coderrupee.ai/docs/sdk) para obtener más detalles.
...
```ts title=".coderrupee/plugins/compaction.ts"
import type { Plugin } from "@coderrupee/plugin"
...
```ts title=".coderrupee/plugins/custom-compaction.ts"
import type { Plugin } from "@coderrupee/plugin"
