...
Las habilidades del agente permiten a CoderRupee descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
CoderRupee busca estas ubicaciones:

- Configuración del proyecto: `.coderrupee/skills/<name>/SKILL.md`
- Configuración global: `~/.config/coderrupee/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, CoderRupee sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.coderrupee/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/coderrupee/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.coderrupee/skills/git-release/SKILL.md` así:
...
CoderRupee enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `coderrupee.json`:
...
**Para agentes integrados** (en `coderrupee.json`):
