---
title: TUI
description: Using the CoderRupee terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CoderRupee provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running CoderRupee starts the TUI for the current directory.
...
When using the CoderRupee TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to CoderRupee. Allows you to select from available providers and add their API keys.
...
Exit CoderRupee. _Aliases_: `/quit`, `/q`
...
This is separate from `coderrupee.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long CoderRupee waits after the leader key. Defaults to `2000`.
...
Use `CODERRUPEE_TUI_CONFIG` to load a custom TUI config path.
...
- `sound_pack` - Sound pack ID to use. Defaults to `coderrupee.default`.
