---
title: Eklentiler
description: CoderRupee'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek CoderRupee'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya CoderRupee'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.coderrupee/plugins/` - Proje düzeyinde eklentiler
- `~/.config/coderrupee/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/coderrupee/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/coderrupee/coderrupee.json`)
2. Project config (`coderrupee.json`)
3. Global eklenti dizini (`~/.config/coderrupee/plugins/`)
4. Proje eklenti dizini (`.coderrupee/plugins/`)
...
CoderRupee bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir CoderRupee SDK istemcisi.
...
CoderRupee'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
CoderRupee masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
CoderRupee'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca CoderRupee'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, CoderRupee'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra CoderRupee'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://coderrupee.ai/docs/sdk) bakın.
