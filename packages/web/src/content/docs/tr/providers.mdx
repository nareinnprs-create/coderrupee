---
title: Sağlayıcılar
description: coderrupee'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

coderrupee, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı coderrupee yapılandırmanızda yapılandırın.
...
`~/.local/share/coderrupee/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları coderrupee'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
CoderRupee Zen, coderrupee ekibi tarafından sağlanan modellerin bir listesidir.
coderrupee ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz CoderRupee Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, coderrupee'u seçin ve [coderrupee.ai/auth](https://coderrupee.ai/auth)'ye gidin.
...
coderrupee'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## CoderRupee Go

CoderRupee Go, coderrupee ile iyi çalıştığı test edilmiş ve doğrulanmış, coderrupee ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `CoderRupee Go`'yu seçin ve [coderrupee.ai/auth](https://coderrupee.ai/zen) adresine gidin.
...
Amazon Bedrock'u coderrupee ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   coderrupee'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `coderrupee.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi coderrupee'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
coderrupee'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   coderrupee'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
:::note[uyumluluk notu]
coderrupee, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. coderrupee'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`coderrupee.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [coderrupee-gitlab-auth](https://www.npmjs.com/package/coderrupee-gitlab-auth) ana sayfasında.

##### Yapılandırma

`coderrupee.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi coderrupee'la kullanmak için:
...
Google Vertex AI'yi coderrupee ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları coderrupee çalıştırılırken ayarlayın.
...
Helicone'un coderrupee aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her coderrupee görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [coderrupee-helicone-session](https://github.com/H2Shami/coderrupee-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her coderrupee görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
coderrupee'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla coderrupee'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla coderrupee'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini coderrupee için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/coderrupee)'a bakın.
:::
...
Ollama Cloud'u coderrupee ile kullanmak için:
...
3. coderrupee'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: coderrupee'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
CoderRupee Zen, coderrupee ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>CoderRupee Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **CoderRupee Zen**'i arayın.

   ```txt
   /connect
   ```

3. coderrupee API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   coderrupee yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca coderrupee yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
Modelleri coderrupee yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı coderrupee ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `coderrupee.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları coderrupee'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `coderrupee auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için coderrupee yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, coderrupee yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
