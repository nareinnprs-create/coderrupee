...
CoderRupee Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli **aylık 10$ aboneliğidir**.

Go, CoderRupee'daki diğer sağlayıcılar gibi çalışır. CoderRupee Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve CoderRupee'u kullanmak için bunu kullanmanıza gerek yoktur.
...
CoderRupee Go, bu modellere **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

CoderRupee Go, CoderRupee'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>CoderRupee Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `CoderRupee Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye CoderRupee Go'ya abone olabilir.
:::
...
CoderRupee Go aşağıdaki limitleri içerir:
...
CoderRupee yapılandırmanızdaki [model id](/docs/config/#models) formatı `coderrupee-go/<model-id>` şeklindedir. Örneğin, Kimi K3 için yapılandırmanızda `coderrupee-go/kimi-k3` kullanmalısınız.
...
CoderRupee Go'yu şu amaçlarla oluşturduk:
...
4. CoderRupee ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
