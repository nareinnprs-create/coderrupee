...
coderrupee ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak coderrupee kendi `coderrupee` temasını kullanır.
...
coderrupee birden fazla yerleşik temayla gelir.
...
- coderrupee'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
coderrupee, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/coderrupee/themes/*.json` veya `$XDG_CONFIG_HOME/coderrupee/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.coderrupee/themes/*.json`
4. **Geçerli çalışma dizini** - `./.coderrupee/themes/*.json`
