...
coderrupee'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
coderrupee, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
CoderRupee, **General**, **Explore** ve **Scout** olmak üzere üç yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

CoderRupee iki yerleşik birincil agent ve üç yerleşik alt agent ile birlikte gelir.
...
Agent'ları `coderrupee.json` yapılandırma dosyanızda yapılandırın:
...
- Proje başına: `.coderrupee/agents/`
...
Sıcaklık belirtilmezse coderrupee modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global coderrupee yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
coderrupee yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [CoderRupee Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `coderrupee/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `coderrupee models` komutunu çalıştırın.
:::
...
:::tip
Paylaşmak istediğiniz bir agent'ınız var mı? [Submit a PR](https://github.com/coderrupee/coderrupee).
:::
