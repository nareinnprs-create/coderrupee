---
title: Yapılandırma
description: coderrupee JSON yapılandırması kullanır.
---

coderrupee'u JSON yapılandırma dosyası kullanarak yapılandırabilirsiniz.

---

## Format

coderrupee hem **JSON** hem de **JSONC** (JSON Yorumlarla birlikte) formatlarını destekler.
...
Örneğin, genel ayarınız `theme: "coderrupee"` ve `autoupdate: true`'yi ayarlıyorsa ve proje ayarlarınız `model: "anthropic/claude-sonnet-4-5"`'yi ayarlıyorsa, nihai yapılandırma üç ayarı da içerecektir.
...
1. **Uzak yapılandırma** (`.well-known/coderrupee`'dan) - kurumsal varsayılanlar
2. **Genel yapılandırma** (`~/.config/coderrupee/coderrupee.json`) - kullanıcı tercihleri
3. **Özel yapılandırma** (`CODERRUPEE_CONFIG` env var) - özel geçersiz kılmalar
4. **Proje yapılandırması** (projedeki `coderrupee.json`) - projeye özgü ayarlar
5. **`.coderrupee` dizinleri** - agent'lar, komutlar, eklentiler
6. **Satır içi yapılandırma** (`CODERRUPEE_CONFIG_CONTENT` env var) - çalışma zamanı geçersiz kılmaları
...
:::note
`.coderrupee` ve `~/.config/coderrupee` dizinleri alt dizinler için **çoğul adlar** kullanır: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` ve `themes/`. Tekil adlar (örneğin, `agent/`) da geriye dönük uyumluluk açısından desteklenir.
:::
...
Kuruluşlar, `.well-known/coderrupee` uç noktası aracılığıyla varsayılan yapılandırmayı sağlayabilir. Bu, onu destekleyen bir sağlayıcıyla kimlik doğrulaması yaptığınızda otomatik olarak alınır.
...
Global coderrupee yapılandırmanızı `~/.config/coderrupee/coderrupee.json` içine yerleştirin. Temalar, sağlayıcılar veya tuş atamaları gibi kullanıcı çapındaki tercihler için genel yapılandırmayı kullanın.
...
Proje kökünüze `coderrupee.json` ekleyin. Proje yapılandırması, standart yapılandırma dosyaları arasında en yüksek önceliğe sahiptir; hem genel hem de uzak yapılandırmaları geçersiz kılar.

:::tip
Projeye özel yapılandırmayı projenizin köküne yerleştirin.
:::

coderrupee başlatıldığında geçerli dizinde bir yapılandırma dosyası arar veya en yakın Git dizinine gider.
...
`CODERRUPEE_CONFIG` ortam değişkenini kullanarak özel bir yapılandırma dosyası yolu belirtin.
...
`CODERRUPEE_CONFIG_DIR` kullanarak özel bir yapılandırma dizini belirtin
ortam değişkeni. Bu dizin agent'lar, komutlar için aranacaktır.
modlar ve eklentiler tıpkı standart `.coderrupee` dizini gibi olmalıdır ve
...
Özel dizin, genel yapılandırma ve `.coderrupee` dizinlerinden sonra yüklenir, böylece bunların ayarlarını **geçersiz kılabilir**.

---

## Şema

Yapılandırma dosyası [**`coderrupee.ai/config.json`**](https://coderrupee.ai/config.json)'da tanımlanan bir şemaya sahiptir.
...
`coderrupee serve` ve `coderrupee web` komutları için sunucu ayarlarını `server` seçeneği aracılığıyla yapılandırabilirsiniz.
...
- `mdns` - mDNS hizmet bulmayı etkinleştirin. Bu, ağdaki diğer cihazların coderrupee sunucunuzu keşfetmesine olanak tanır.
- `mdnsDomain` - mDNS hizmeti için özel alan adı. Varsayılan olarak `coderrupee.local` şeklindedir. Aynı ağda birden fazla örneği çalıştırmak için kullanışlıdır.
...
coderrupee yapılandırmanızda kullanmak istediğiniz sağlayıcıları ve modelleri `provider`, `model` ve `small_model` seçenekleri aracılığıyla yapılandırabilirsiniz.
...
`small_model` seçeneği, başlık oluşturma gibi hafif görevler için ayrı bir model yapılandırır. Varsayılan olarak coderrupee, sağlayıcınızda mevcutsa daha ucuz bir model kullanmaya çalışır, aksi takdirde ana modelinize geri döner.
...
coderrupee yapılandırmanızda kullanmak istediğiniz temayı `theme` seçeneği aracılığıyla yapılandırabilirsiniz.
...
Agent'ları `~/.config/coderrupee/agents/` veya `.coderrupee/agents/` içindeki Markdown dosyalarıyla da tanımlayabilirsiniz. [Daha fazla bilgi](/docs/agents).
...
Varsayılan agent, birincil agent olmalıdır (alt agent değil). Bu, `"build"` veya `"plan"` gibi yerleşik bir agent ya da tanımladığınız bir [özel agent](/docs/agents) olabilir. Belirtilen agent mevcut değilse veya bir alt agent ise, coderrupee bir uyarıyla birlikte `"build"`'ye geri döner.

Bu ayar tüm arayüzler için geçerlidir: TUI, CLI (`coderrupee run`), masaüstü uygulaması ve GitHub Action.
...
Komutları `~/.config/coderrupee/commands/` veya `.coderrupee/commands/` içindeki Markdown dosyalarıyla da tanımlayabilirsiniz. [Daha fazla bilgi](/docs/commands).
...
coderrupee başlatıldığında yeni güncellemeleri otomatik olarak indirecektir. Bunu `autoupdate` seçeneğiyle devre dışı bırakabilirsiniz.
...
Varsayılan olarak, coderrupee açık bir onay gerektirmeden **tüm işlemlere izin verir**. `permission` seçeneğini kullanarak bunu değiştirebilirsiniz.
...
[Eklentiler](/docs/plugins) coderrupee'u özel araçlar, kancalar ve entegrasyonlarla genişletin.

Eklenti dosyalarını `.coderrupee/plugins/` veya `~/.config/coderrupee/plugins/` içine yerleştirin. Ayrıca eklentileri `plugin` seçeneği aracılığıyla npm'den de yükleyebilirsiniz.
...
Bu, coderrupee'u tek tek devre dışı bırakmak yerine yalnızca belirli sağlayıcıları kullanacak şekilde kısıtlamak istediğinizde kullanışlıdır.
