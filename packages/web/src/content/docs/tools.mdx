...
Tools allow the LLM to perform actions in your codebase. CoderRupee comes with a set of built-in tools, but you can extend it with [custom tools](/docs/custom-tools) or [MCP servers](/docs/mcp-servers).
...
Here are all the built-in tools available in CoderRupee.
...
:::note
This tool is only available when `CODERRUPEE_EXPERIMENTAL_LSP_TOOL=true` (or `CODERRUPEE_EXPERIMENTAL=true`).
:::
...
:::note
This tool is only available when using the CoderRupee or CoderRupee Go provider, or when either the `CODERRUPEE_ENABLE_EXA` or `CODERRUPEE_ENABLE_PARALLEL` environment variable is set to any truthy value (e.g., `true` or `1`).

To enable when launching CoderRupee:

```bash
CODERRUPEE_ENABLE_EXA=1 coderrupee
# or
CODERRUPEE_ENABLE_PARALLEL=1 coderrupee
```
