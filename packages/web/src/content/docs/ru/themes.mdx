...
С помощью coderrupee вы можете выбрать одну из нескольких встроенных тем, использовать тему, которая адаптируется к теме вашего терминала, или определить свою собственную тему.

По умолчанию coderrupee использует нашу собственную тему `coderrupee`.
...
coderrupee поставляется с несколькими встроенными темами.
...
- Хотите, чтобы coderrupee соответствовал внешнему виду их терминала
...
coderrupee поддерживает гибкую систему тем на основе JSON, которая позволяет пользователям легко создавать и настраивать темы.
...
2. **Каталог конфигурации пользователя** – определяется в `~/.config/coderrupee/themes/*.json` или `$XDG_CONFIG_HOME/coderrupee/themes/*.json`.
3. **Корневой каталог проекта** – определено в `<project-root>/.coderrupee/themes/*.json`.
4. **Текущий рабочий каталог** – определено в `./.coderrupee/themes/*.json`.
