...
Навыки агента позволяют coderrupee обнаруживать многократно используемые инструкции из вашего репозитория или домашнего каталога.
...
coderrupee выполняет поиск в следующих местах:

- Конфигурация проекта: `.coderrupee/skills/<name>/SKILL.md`
- Глобальная конфигурация: `~/.config/coderrupee/skills/<name>/SKILL.md`.
...
Для локальных путей проекта coderrupee переходит из вашего текущего рабочего каталога, пока не достигнет рабочего дерева git.
Он загружает все соответствующие `skills/*/SKILL.md` в `.coderrupee/` и все соответствующие `.claude/skills/*/SKILL.md` или `.agents/skills/*/SKILL.md` по пути.

Глобальные определения также загружаются из `~/.config/coderrupee/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` и `~/.agents/skills/*/SKILL.md`.
...
Создайте `.coderrupee/skills/git-release/SKILL.md` следующим образом:
...
coderrupee перечисляет доступные навыки в описании инструмента `skill`.
...
Контролируйте, к каким навыкам агенты могут получить доступ, используя разрешения на основе шаблонов в `coderrupee.json`:
...
**Для встроенных агентов** (в формате `coderrupee.json`):
