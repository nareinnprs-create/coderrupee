...
Создайте `.coderrupee/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию coderrupee или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем coderrupee [config](/docs/config):
...
- Глобальный: `~/.config/coderrupee/commands/`
- Для каждого проекта: `.coderrupee/commands/`
...
coderrupee включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
