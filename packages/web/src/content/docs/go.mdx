...
CoderRupee Go is a low cost **$10/month subscription** that gives you reliable access to popular open coding models.

Go works like any other provider in CoderRupee. You subscribe to CoderRupee Go and
...
CoderRupee Go gives you access to these models for **$10/month**.

---

## How it works

CoderRupee Go works like any other provider in CoderRupee.

1. You sign in to **<a href={console}>CoderRupee Zen</a>**, subscribe to Go, and
   copy your API key.
2. You run the `/connect` command in the TUI, select `CoderRupee Go`, and paste
   your API key.
3. Run `/models` in the TUI to see the list of models available through Go.

:::note
Only one member per workspace can subscribe to CoderRupee Go.
:::
...
CoderRupee Go includes the following limits:
...
uses the format `coderrupee-go/<model-id>`. For example, for Kimi K3, you would
use `coderrupee-go/kimi-k3` in your config.
...
4. Have **no lock-in** by allowing you to use any other provider with CoderRupee as well.
