...
Con CoderRupee puoi scegliere tra diversi temi integrati, usare un tema che si adatta al tema del tuo terminale oppure definire un tema personalizzato.

Per impostazione predefinita, CoderRupee usa il tema `coderrupee`.
...
CoderRupee include diversi temi integrati.
...
- Vuole che CoderRupee corrisponda all'aspetto del terminale
...
CoderRupee supporta un sistema di temi flessibile basato su JSON che permette di creare e personalizzare temi facilmente.
...
2. **Directory di configurazione utente** - in `~/.config/coderrupee/themes/*.json` o `$XDG_CONFIG_HOME/coderrupee/themes/*.json`
3. **Directory root del progetto** - in `<project-root>/.coderrupee/themes/*.json`
4. **Directory di lavoro corrente** - in `./.coderrupee/themes/*.json`
