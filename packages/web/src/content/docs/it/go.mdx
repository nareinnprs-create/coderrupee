...
CoderRupee Go è un abbonamento a basso costo da **10 $/mese** che ti offre un accesso affidabile ai popolari modelli di programmazione aperti.

Go funziona come qualsiasi altro provider in CoderRupee. Ti abboni a CoderRupee Go e
ottieni la tua chiave API. È **completamente facoltativo** e non hai bisogno di usarlo per
utilizzare CoderRupee.
...
CoderRupee Go ti dà accesso a questi modelli a **10 $/mese**.

---

## Come funziona

CoderRupee Go funziona come qualsiasi altro provider in CoderRupee.

1. Accedi a **<a href={console}>CoderRupee Zen</a>**, ti abboni a Go e
   copi la tua chiave API.
2. Esegui il comando `/connect` nella TUI, selezioni `CoderRupee Go` e incolli
   la tua chiave API.
3. Esegui `/models` nella TUI per vedere l'elenco dei modelli disponibili tramite Go.

:::note
Solo un membro per workspace può abbonarsi a CoderRupee Go.
:::
...
CoderRupee Go include i seguenti limiti:
...
Il [model id](/docs/config/#models) nella tua CoderRupee config
utilizza il formato `coderrupee-go/<model-id>`. Ad esempio, per Kimi K3, useresti
`coderrupee-go/kimi-k3` nella tua configurazione.
...
Abbiamo creato CoderRupee Go per:
...
4. Non avere **nessun lock-in**, permettendoti di utilizzare anche qualsiasi altro provider con CoderRupee.
