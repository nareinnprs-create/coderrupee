...
Puoi aggiungere strumenti esterni a CoderRupee usando il _Model Context Protocol_, o MCP. CoderRupee supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [CoderRupee Config](https://coderrupee.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/coderrupee`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
CoderRupee gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, CoderRupee:
...
Se il server richiede autenticazione, CoderRupee ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `coderrupee mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, CoderRupee memorizzera' i token in modo sicuro in `~/.local/share/coderrupee/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in CoderRupee insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione CoderRupee come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare CoderRupee al tuo account Sentry.
