---
title: Server LSP
description: CoderRupee si integra con i tuoi server LSP.
---

CoderRupee può integrarsi con server Language Server Protocol (LSP) per usare la diagnostica come feedback per l'agente.

---

## Integrati

CoderRupee include diversi server LSP integrati per linguaggi popolari:
...
:::note
Puoi disabilitare il download automatico dei server LSP impostando la variabile d'ambiente `CODERRUPEE_DISABLE_LSP_DOWNLOAD` a `true`.
:::
...
Quando LSP è abilitato e coderrupee apre un file:
...
Puoi abilitare e personalizzare i server LSP tramite la sezione `lsp` nella configurazione di coderrupee.
