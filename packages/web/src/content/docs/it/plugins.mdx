---
title: Plugin
description: Scrivi plugin per estendere CoderRupee.
---

I plugin ti permettono di estendere CoderRupee agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di CoderRupee.
...
- `.coderrupee/plugins/` - plugin a livello progetto
- `~/.config/coderrupee/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/coderrupee/node_modules/`.
...
1. Config globale (`~/.config/coderrupee/coderrupee.json`)
2. Config di progetto (`coderrupee.json`)
3. Directory plugin globale (`~/.config/coderrupee/plugins/`)
4. Directory plugin di progetto (`.coderrupee/plugins/`)
...
CoderRupee esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di coderrupee per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere coderrupee.
...
:::note
Se usi l'app desktop di CoderRupee, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a coderrupee di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a coderrupee:
...
L'helper `tool` crea uno strumento personalizzato che coderrupee può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in coderrupee insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://coderrupee.ai/docs/sdk) per i dettagli.
