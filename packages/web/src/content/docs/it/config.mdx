...
description: Usa la configurazione JSON di CoderRupee.
---

Puoi configurare CoderRupee usando un file di configurazione JSON.

---

## Formato

CoderRupee supporta sia **JSON** sia **JSONC** (JSON con commenti).
...
1. **Config remota** (da `.well-known/coderrupee`) - default dell'organizzazione
2. **Config globale** (`~/.config/coderrupee/coderrupee.json`) - preferenze utente
3. **Config personalizzata** (variabile d'ambiente `CODERRUPEE_CONFIG`) - sovrascritture personalizzate
4. **Config di progetto** (`coderrupee.json` nel progetto) - impostazioni specifiche del progetto
5. **Directory `.coderrupee`** - agenti, comandi, plugin
6. **Config inline** (variabile d'ambiente `CODERRUPEE_CONFIG_CONTENT`) - sovrascritture a runtime
...
:::note
Le directory `.coderrupee` e `~/.config/coderrupee` usano **nomi al plurale** per le sottodirectory: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` e `themes/`. I nomi al singolare (ad es. `agent/`) sono supportati per retrocompatibilita.
:::
...
Le organizzazioni possono fornire una configurazione predefinita tramite l'endpoint `.well-known/coderrupee`. Viene recuperata automaticamente quando ti autentichi con un provider che lo supporta.
...
Metti la configurazione globale di CoderRupee in `~/.config/coderrupee/coderrupee.json`. Usa la configurazione globale per preferenze server/runtime valide per l'utente come provider, modelli e permessi.

Per le impostazioni specifiche della TUI, usa `~/.config/coderrupee/tui.json`.
...
Aggiungi `coderrupee.json` nella root del progetto. La configurazione di progetto ha la precedenza piu alta tra i file standard: sovrascrive sia la configurazione globale sia quella remota.
...
Quando CoderRupee si avvia, cerca un file di configurazione nella directory corrente oppure risale fino alla directory Git piu vicina.
...
Specifica un percorso personalizzato per il file di configurazione usando la variabile d'ambiente `CODERRUPEE_CONFIG`.
...
Specifica una directory di configurazione personalizzata usando la variabile d'ambiente `CODERRUPEE_CONFIG_DIR`. Questa directory verra usata per cercare agenti, comandi, modalita e plugin proprio come la directory standard `.coderrupee` e dovrebbe seguire la stessa struttura.
...
La directory personalizzata viene caricata dopo la configurazione globale e le directory `.coderrupee`, quindi **puo sovrascrivere** le loro impostazioni.

---

## Schema

Il file di configurazione ha uno schema definito in [**`coderrupee.ai/config.json`**](https://coderrupee.ai/config.json).

La configurazione TUI usa [**`coderrupee.ai/tui.json`**](https://coderrupee.ai/tui.json).
...
Usa `CODERRUPEE_TUI_CONFIG` per puntare a un file di configurazione TUI personalizzato.

Le chiavi legacy `theme`, `keybinds` e `tui` in `coderrupee.json` sono deprecate e vengono migrate automaticamente quando possibile.
...
Puoi configurare le impostazioni del server per i comandi `coderrupee serve` e `coderrupee web` tramite l'opzione `server`.
...
- `mdns` - Abilita la scoperta del servizio mDNS. Questo permette ad altri dispositivi sulla rete di scoprire il tuo server CoderRupee.
- `mdnsDomain` - Nome di dominio personalizzato per il servizio mDNS. Predefinito: `coderrupee.local`. Utile per eseguire piu istanze sulla stessa rete.
...
Puoi configurare provider e modelli da usare in CoderRupee tramite le opzioni `provider`, `model` e `small_model`.
...
L'opzione `small_model` configura un modello separato per task leggeri come la generazione dei titoli. Per impostazione predefinita, CoderRupee prova a usare un modello più economico se disponibile dal tuo provider, altrimenti fa fallback sul tuo modello principale.
...
Puoi anche definire agenti usando file markdown in `~/.config/coderrupee/agents/` o `.coderrupee/agents/`. [Scopri di piu](/docs/agents).
...
L'agente predefinito deve essere un agente primary (non un subagent). Puo essere un agente integrato come `"build"` o `"plan"`, oppure un [agente personalizzato](/docs/agents) che hai definito. Se l'agente specificato non esiste o e un subagent, CoderRupee fara fallback su `"build"` mostrando un avviso.

Questa impostazione si applica a tutte le interfacce: TUI, CLI (`coderrupee run`), app desktop e GitHub Action.
...
Puoi anche definire comandi usando file markdown in `~/.config/coderrupee/commands/` o `.coderrupee/commands/`. [Scopri di piu](/docs/commands).
...
CoderRupee scarichera automaticamente eventuali aggiornamenti quando si avvia. Puoi disabilitare questa funzione con l'opzione `autoupdate`.
...
Per impostazione predefinita, coderrupee **consente tutte le operazioni** senza richiedere un'approvazione esplicita. Puoi cambiarlo usando l'opzione `permission`.
...
[I plugin](/docs/plugins) estendono CoderRupee con strumenti personalizzati, hook e integrazioni.

Metti i file dei plugin in `.coderrupee/plugins/` o `~/.config/coderrupee/plugins/`. Puoi anche caricare plugin da npm tramite l'opzione `plugin`.
...
E utile quando vuoi limitare CoderRupee a usare solo alcuni provider invece di disabilitarli uno per uno.
