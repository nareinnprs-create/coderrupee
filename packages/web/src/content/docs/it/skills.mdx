...
Le skill degli agenti permettono a CoderRupee di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
CoderRupee cerca in queste posizioni:

- Config di progetto: `.coderrupee/skills/<name>/SKILL.md`
- Config globale: `~/.config/coderrupee/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, CoderRupee risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.coderrupee/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/coderrupee/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.coderrupee/skills/git-release/SKILL.md` cosi':
...
CoderRupee elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `coderrupee.json`:
...
**Per agenti integrati** (in `coderrupee.json`):
