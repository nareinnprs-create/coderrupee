...
Per diagnosticare problemi con CoderRupee, inizia controllando i log e i dati locali che salva su disco.
...
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\coderrupee\log`

I file di log hanno nomi con timestamp (ad es. `2025-01-09T123456.log`) e vengono conservati i 10 file di log piu recenti.

Puoi impostare il livello di log con l'opzione a riga di comando `--log-level` per ottenere informazioni di debug piu dettagliate. Per esempio: `coderrupee --log-level DEBUG`.

---

## Archiviazione

coderrupee salva i dati delle sessioni e altri dati dell'applicazione su disco in:

- **macOS/Linux**: `~/.local/share/coderrupee/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\coderrupee`
...
CoderRupee Desktop esegue in background un server locale di CoderRupee (il sidecar `coderrupee-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
- Solo macOS: menu `CoderRupee` -> **Reload Webview** (utile se l'interfaccia e vuota o bloccata).
...
- **macOS/Linux** (installazioni vecchie): `~/.local/share/coderrupee/coderrupee.jsonc`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\coderrupee\coderrupee.jsonc`
...
CoderRupee puo anche caricare plugin locali dal disco. Spostali temporaneamente altrove (o rinomina la cartella) e riavvia l'app desktop:

- **Plugin globali**
  - **macOS/Linux**: `~/.config/coderrupee/plugins/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\coderrupee\plugins`
...
Se disabilitare i plugin non aiuta (o l'installazione di un plugin e bloccata), svuota la cache in modo che CoderRupee possa ricostruirla.

1. Chiudi completamente CoderRupee Desktop.
2. Elimina la directory della cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/coderrupee`
- **Linux**: elimina `~/.cache/coderrupee` (oppure esegui `rm -rf ~/.cache/coderrupee`)
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.cache\coderrupee`

3. Riavvia CoderRupee Desktop.

---

### Risolvi problemi di connessione al server

CoderRupee Desktop puo avviare il proprio server locale (predefinito) oppure connettersi a un URL server che hai configurato.
...
Se il tuo `coderrupee.json(c)` contiene una sezione `server`, rimuovila temporaneamente e riavvia l'app desktop.

#### Controlla le variabili d'ambiente

Se hai `CODERRUPEE_PORT` impostato nell'ambiente, l'app desktop provera a usare quella porta per il server locale.

- Rimuovi `CODERRUPEE_PORT` (o scegli una porta libera) e riavvia.
...
Su Windows, CoderRupee Desktop richiede **WebView2 Runtime** di Microsoft Edge. Se l'app si apre su una finestra vuota o non parte, installa/aggiorna WebView2 e riprova.

---

### Windows: problemi generali di prestazioni

Se riscontri prestazioni lente, problemi di accesso ai file o problemi del terminale su Windows, prova a usare [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL fornisce un ambiente Linux che funziona in modo piu fluido con le funzionalita di CoderRupee.

---

### Notifiche non visualizzate

CoderRupee Desktop mostra le notifiche di sistema solo quando:

- le notifiche sono abilitate per CoderRupee nelle impostazioni del sistema operativo, e
...
1. Chiudi CoderRupee Desktop.
2. Trova ed elimina questi file (si trovano nella directory dati dell'app CoderRupee Desktop):

- `coderrupee.settings.dat` (desktop default server URL)
- `coderrupee.global.dat` e `coderrupee.workspace.*.dat` (stato dell'interfaccia come server/progetti recenti)
...
Se riscontri problemi con CoderRupee:
...
Per capire a quali modelli hai accesso, esegui `coderrupee models`
...
2. Se il problema persiste, prova a cancellare la configurazione salvata:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.local\share\coderrupee`
...
Se incontri errori nelle chiamate API, potrebbe dipendere da pacchetti provider non aggiornati. coderrupee installa dinamicamente i pacchetti provider (OpenAI, Anthropic, Google, ecc.) quando servono e li mette in cache localmente.
...
1. Svuota la cache dei pacchetti provider:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.cache\coderrupee`

2. Riavvia coderrupee per reinstallare i pacchetti provider piu recenti

Questo forzera coderrupee a scaricare le versioni piu recenti dei pacchetti provider, cosa che spesso risolve problemi di compatibilita con parametri dei modelli e cambiamenti delle API.
...
coderrupee rilevera se stai usando Wayland e preferira `wl-clipboard`; altrimenti provera a trovare gli strumenti per gli appunti nell'ordine: `xclip` e `xsel`.
