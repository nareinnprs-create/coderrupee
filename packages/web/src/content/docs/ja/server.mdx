---
title: サーバー
description: HTTP 経由で CoderRupee サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`coderrupee serve` コマンドは、CoderRupee クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `coderrupee.local` |
...
HTTP 基本認証でサーバーを保護するには、`CODERRUPEE_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `coderrupee` になるか、`CODERRUPEE_SERVER_USERNAME` を設定してオーバーライドします。これは、`coderrupee serve` と `coderrupee web` の両方に当てはまります。
...
`coderrupee` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
CoderRupee サーバーを使用して、プログラムで CoderRupee と対話します。
:::

`coderrupee serve` を実行してスタンドアロンサーバーを起動できます。すでに CoderRupee TUI を実行している場合でも、`coderrupee serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、CoderRupee [IDE](/docs/ide) プラグインによって使用されます。
