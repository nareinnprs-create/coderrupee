---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i CoderRupee.
---

import config from "../../../../config.mjs"
export const console = config.console

CoderRupee bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i CoderRupee-konfigurasjonen.
...
i `~/.local/share/coderrupee/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i CoderRupee
konfig.
...
CoderRupee Zen er en liste over modeller levert av CoderRupee-teamet som har vært
testet og verifisert for å fungere godt med CoderRupee. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med CoderRupee Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `CoderRupee Zen` og gå til [coderrupee.ai/auth](https://coderrupee.ai/zen).
...
Det fungerer som alle andre leverandører i CoderRupee og er helt valgfritt å bruke.

---

## CoderRupee Go

CoderRupee Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av CoderRupee-teamet som har vært
testet og verifisert for å fungere godt med CoderRupee.

1. Kjør kommandoen `/connect` i TUI, velg `CoderRupee Go`, og gå til [coderrupee.ai/auth](https://coderrupee.ai/zen).
...
Slik bruker du Amazon Bedrock med CoderRupee:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører CoderRupee:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `coderrupee.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i coderrupee støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere coderrupee til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at CoderRupee skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom CoderRupee-konfigurasjonen.
...
for å lagre token i CoderRupee auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
CoderRupee bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse CoderRupee
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`coderrupee.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [coderrupee-gitlab-auth](https://www.npmjs.com/package/coderrupee-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `coderrupee.json`:
...
Slik bruker du GitHub Copilot-abonnementet med CoderRupee:
...
Slik bruker du Google Vertex AI med CoderRupee:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører CoderRupee.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom CoderRupee, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [coderrupee-helicone-session](https://github.com/H2Shami/coderrupee-helicone-session) plugin for automatisk å logge hver CoderRupee-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver CoderRupee-samtale oppført som en separat økt.
...
Du kan konfigurere CoderRupee for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere CoderRupee for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere CoderRupee for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for CoderRupee. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/coderrupee) for detaljer.
:::
...
Slik bruker du Ollama Cloud med CoderRupee:
...
3. Kopier API-nøkkelen for bruk i CoderRupee.
...
6. **Viktig**: Før du bruker skymodeller i CoderRupee, må du hente modellinformasjonen lokalt:
...
CoderRupee Zen er en liste over testede og verifiserte modeller levert av CoderRupee-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>CoderRupee Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **CoderRupee Zen**.

   ```txt
   /connect
   ```

3. Skriv inn CoderRupee API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom CoderRupee-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom CoderRupee-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med CoderRupee:
...
Du kan også tilpasse modeller gjennom CoderRupee-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med CoderRupee. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `coderrupee.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar CoderRupee forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `coderrupee auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk CoderRupee-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i CoderRupee-konfigurasjonen.
