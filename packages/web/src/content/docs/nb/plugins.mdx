...
description: Skriv dine egne programtillegg for å utvide CoderRupee.
---

Plugins lar deg utvide CoderRupee ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til CoderRupee.
...
- `.coderrupee/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/coderrupee/node_modules/`.
...
1. Global konfigurasjon (`~/.config/coderrupee/coderrupee.json`)
2. Prosjektkonfigurasjon (`coderrupee.json`)
3. Global plugin-katalog (`~/.config/coderrupee/plugins/`)
4. Prosjektpluginkatalog (`.coderrupee/plugins/`)
...
CoderRupee kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En CoderRupee SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide CoderRupee.
...
:::note
Hvis du bruker CoderRupee-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre CoderRupee fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til CoderRupee:
...
`tool`-hjelperen lager et tilpasset verktøy som CoderRupee kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for CoderRupee sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://coderrupee.ai/docs/sdk) for detaljer.
