...
Du kan legge til eksterne verktøy i CoderRupee ved å bruke _Model Context Protocol_ eller MCP. CoderRupee støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [CoderRupee Config](https://coderrupee.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="coderrupee.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/coderrupee`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
```jsonc title="coderrupee.jsonc" {15}
{
  "$schema": "https://coderrupee.ai/config.json",
  "mcp": {
    "my-local-mcp-server": {
...
CoderRupee håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil CoderRupee:
...
Hvis serveren krever autentisering, vil CoderRupee be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `coderrupee mcp auth <server-name>`.
...
```json title="coderrupee.json" {7-11}
{
  "$schema": "https://coderrupee.ai/config.json",
  "mcp": {
    "my-oauth-server": {
...
```bash
coderrupee mcp auth my-oauth-server
```
...
```bash
coderrupee mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil CoderRupee lagre tokens sikkert i `~/.local/share/coderrupee/mcp-auth.json`.
...
coderrupee mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i CoderRupee, sammen med innebygde verktøy. Så du kan administrere dem gjennom CoderRupee-konfigurasjonen som alle andre verktøy.
...
```json title="coderrupee.json" {11, 14-18}
{
  "$schema": "https://coderrupee.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```json title="coderrupee.json" {4-8}
{
  "$schema": "https://coderrupee.ai/config.json",
  "mcp": {
    "sentry": {
...
```bash
coderrupee mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble CoderRupee til Sentry-kontoen din.
...
```json title="coderrupee.json" {4-7}
{
  "$schema": "https://coderrupee.ai/config.json",
  "mcp": {
    "context7": {
...
```json title="coderrupee.json" {7-9}
{
  "$schema": "https://coderrupee.ai/config.json",
  "mcp": {
    "context7": {
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver CoderRupee-samtale oppført som en separat økt.
