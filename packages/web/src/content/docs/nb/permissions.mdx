...
CoderRupee bruker `permission`-konfigurasjonen for å bestemme om en gitt handling skal kjøres automatisk, spørre deg eller blokkeres.
...
Bruk `external_directory` for å tillate verktøyanrop som berører stier utenfor arbeidskatalogen der CoderRupee ble startet. Dette gjelder alle verktøy som tar en bane som input (for eksempel `read`, `edit`, `glob`, `grep` og mange `bash`-kommandoer).
...
CoderRupee-tillatelser tastes inn etter verktøynavn, pluss et par sikkerhetsvakter:
...
Hvis du ikke spesifiserer noe, starter CoderRupee fra tillatte standardinnstillinger:
...
Når CoderRupee ber om godkjenning, gir brukergrensesnittet tre alternativer:

- `once` — godkjenne nettopp denne forespørselen
- `always` — godkjenn fremtidige forespørsler som samsvarer med de foreslåtte mønstrene (for resten av den nåværende CoderRupee-økten)
