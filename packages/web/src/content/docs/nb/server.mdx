---
title: Tjener
description: Samhandle med CoderRupee-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `coderrupee serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en CoderRupee-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `coderrupee.local` |
...
Sett `CODERRUPEE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `coderrupee` som standard, eller sett `CODERRUPEE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `coderrupee serve` og `coderrupee web`.
...
Når du kjører `coderrupee` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk CoderRupee-serveren til å samhandle med CoderRupee programmatisk.
:::

Denne arkitekturen lar CoderRupee støtte flere klienter og lar deg samhandle med CoderRupee programmatisk.

Du kan kjøre `coderrupee serve` for å starte en frittstående server. Hvis du har
CoderRupee TUI kjører, vil `coderrupee serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av CoderRupee [IDE](/docs/ide) plugins.
...
coderrupee-serveren viser følgende APIer.
