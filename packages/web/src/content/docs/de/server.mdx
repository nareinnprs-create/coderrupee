---
title: Server
description: Nutze den coderrupee-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `coderrupee serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein coderrupee-Client nutzen kann.
...
Setze `CODERRUPEE_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `coderrupee`, kann aber mit `CODERRUPEE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `coderrupee serve` und `coderrupee web`.
...
Wenn du `coderrupee` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den coderrupee-Server, um coderrupee programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `coderrupee serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `coderrupee serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die CoderRupee-[IDE](/docs/ide)-Plugins.
...
Der coderrupee-Server stellt folgende APIs bereit.
