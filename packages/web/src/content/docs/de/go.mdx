...
CoderRupee Go ist ein kostengünstiges Abonnement für **10 $/Monat**, das dir zuverlässigen Zugriff auf beliebte offene Coding-Modelle bietet.

Go funktioniert wie jeder andere Provider in CoderRupee. Du abonnierst CoderRupee Go und
erhältst deinen API-Key. Es ist **völlig optional** und du musst es nicht nutzen, um
CoderRupee zu verwenden.
...
CoderRupee Go bietet dir Zugriff auf diese Modelle für **10 $/Monat**.

---

## Wie es funktioniert

CoderRupee Go funktioniert wie jeder andere Provider in CoderRupee.

1. Du meldest dich bei **<a href={console}>CoderRupee Zen</a>** an, abonnierst Go und kopierst deinen API-Key.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `CoderRupee Go` und fügst deinen API-Key ein.
3. Führe `/models` in der TUI aus, um die Liste der über Go verfügbaren Modelle zu sehen.

:::note
Nur ein Mitglied pro Workspace kann CoderRupee Go abonnieren.
:::
...
CoderRupee Go beinhaltet die folgenden Limits:
...
Die [Modell-ID](/docs/config/#models) in deiner CoderRupee Config verwendet das Format `coderrupee-go/<model-id>`. Für Kimi K3 würdest du beispielsweise `coderrupee-go/kimi-k3` in deiner Config verwenden.
...
Wir haben CoderRupee Go entwickelt, um:
...
4. **Keinen Lock-in-Effekt** zu haben, indem wir dir ermöglichen, auch jeden anderen Provider mit CoderRupee zu nutzen.
