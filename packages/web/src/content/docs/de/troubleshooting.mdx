...
Wenn CoderRupee Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\coderrupee\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `coderrupee --log-level DEBUG`.

---

## Speicher

coderrupee speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/coderrupee/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\coderrupee`
...
CoderRupee Desktop startet im Hintergrund einen lokalen CoderRupee-Server (`coderrupee-cli`-Sidecar).
...
- Nur macOS: `CoderRupee`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/coderrupee/coderrupee.jsonc` (oder `~/.config/coderrupee/coderrupee.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/coderrupee/coderrupee.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\coderrupee\coderrupee.jsonc`
...
CoderRupee kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/coderrupee/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\coderrupee\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit CoderRupee ihn neu aufbauen kann.

1. Beende CoderRupee Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/coderrupee`
- **Linux**: loesche `~/.cache/coderrupee` (oder fuehre aus: `rm -rf ~/.cache/coderrupee`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\coderrupee`

3. Starte CoderRupee Desktop neu.

---

### Server-Verbindungsprobleme beheben

CoderRupee Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `coderrupee.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `CODERRUPEE_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `CODERRUPEE_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt CoderRupee Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit CoderRupee-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

CoderRupee Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer CoderRupee in den OS-Einstellungen aktiviert sind, und
...
1. Beende CoderRupee Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von CoderRupee Desktop):

- `coderrupee.settings.dat` (desktop default server URL)
- `coderrupee.global.dat` und `coderrupee.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit CoderRupee hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `coderrupee models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\coderrupee`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. coderrupee installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\coderrupee`

2. Starte coderrupee neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt coderrupee, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
coderrupee erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
