...
Agent Skills erlauben CoderRupee, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
CoderRupee sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft CoderRupee vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.coderrupee/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/coderrupee/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.coderrupee/skills/git-release/SKILL.md` zum Beispiel so:
...
CoderRupee listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `coderrupee.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `coderrupee.json`):
