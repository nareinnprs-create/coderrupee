---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um CoderRupee zu erweitern.
---

Mit Plugins können Sie CoderRupee erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von CoderRupee zu ändern.
...
- `.coderrupee/plugins/` – Plugins auf Projektebene
- `~/.config/coderrupee/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/coderrupee/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/coderrupee/coderrupee.json`)
2. Projektkonfiguration (`coderrupee.json`)
3. Globales Plugin-Verzeichnis (`~/.config/coderrupee/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.coderrupee/plugins/`)
...
CoderRupee führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein CoderRupee-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie CoderRupee erweitern können.
...
:::note
Wenn Sie die CoderRupee-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass CoderRupee `.env`-Dateien liest:
...
Plugins können CoderRupee auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das CoderRupee aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für CoderRupee zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://coderrupee.ai/docs/sdk).
