---
title: Regeln
description: Lege eigene Anweisungen fuer coderrupee fest.
---

Du kannst coderrupee ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in coderrupee den Befehl `/init` aus.
...
So kann coderrupee sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht coderrupee sie zu erweitern.
...
coderrupee kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/coderrupee/AGENTS.md` speichern.
Diese gelten in allen coderrupee-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt CoderRupee dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/coderrupee/AGENTS.md` existiert)
...
Beim Start sucht coderrupee in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/coderrupee/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `coderrupee.json` oder in `~/.config/coderrupee/coderrupee.json` angeben.
...
coderrupee parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit coderrupee.json

Empfohlen ist das `instructions`-Feld in der `coderrupee.json`:
...
Du kannst coderrupee explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `coderrupee.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
