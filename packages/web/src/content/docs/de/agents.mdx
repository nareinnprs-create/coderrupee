...
Es gibt zwei Arten von Agenten in CoderRupee; Primäragenten und Subagenten.
...
CoderRupee verfügt über zwei integrierte Primäragenten: **Build** und **Plan**. Auch
...
Konfigurieren Sie Agenten in Ihrer `coderrupee.json`-Konfigurationsdatei:
...
Wenn keine Temperatur angegeben ist, verwendet CoderRupee modellspezifische Standardwerte; typischerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle.
...
Dieser Pfad ist relativ zum Speicherort der Konfigurationsdatei. Dies funktioniert sowohl für die globale CoderRupee-Konfiguration als auch für die projektspezifische Konfiguration.
...
Die Modell-ID in Ihrer CoderRupee-Konfiguration verwendet das Format `provider/model-id`. Wenn Sie beispielsweise [CoderRupee Zen](/docs/zen) verwenden, würden Sie `coderrupee/gpt-5.1-codex` für GPT 5.1 Codex verwenden.
...
:::tip
Führen Sie `coderrupee models` aus, um eine Liste der verfügbaren Modelle anzubieten.
:::
...
:::tip
Haben Sie einen Agenten, den Sie gerne teilen möchten? [Reichen Sie einen PR ein](https://github.com/coderrupee/coderrupee).
:::
