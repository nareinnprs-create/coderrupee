...
Mit CoderRupee kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet CoderRupee das `coderrupee`-Theme.
...
CoderRupee liefert mehrere eingebaute Themes mit.
...
- CoderRupee optisch ans Terminal anpassen wollen
...
CoderRupee unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/coderrupee/themes/*.json` oder `$XDG_CONFIG_HOME/coderrupee/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.coderrupee/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.coderrupee/themes/*.json`
