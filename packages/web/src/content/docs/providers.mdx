---
title: Providers
description: Using any LLM provider in CoderRupee.
---

import config from "../../../config.mjs"
export const console = config.console

CoderRupee uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your CoderRupee config.
...
in `~/.local/share/coderrupee/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your CoderRupee
config.
...
CoderRupee Zen is a list of models provided by the CoderRupee team that have been
tested and verified to work well with CoderRupee. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with CoderRupee Zen.
:::

1. Run the `/connect` command in the TUI, select `CoderRupee Zen`, and head to [coderrupee.ai/auth](https://coderrupee.ai/zen).
...
It works like any other provider in CoderRupee and is completely optional to use.

---

## CoderRupee Go

CoderRupee Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the CoderRupee team that have been
tested and verified to work well with CoderRupee.

1. Run the `/connect` command in the TUI, select `CoderRupee Go`, and head to [coderrupee.ai/auth](https://coderrupee.ai/zen).
...
To use Amazon Bedrock with CoderRupee:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running coderrupee:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY coderrupee

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile coderrupee

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX coderrupee
   ```
   ...
   ***
   ...
   For project-specific or persistent configuration, use `coderrupee.json`:
...
CoderRupee. Anthropic explicitly prohibits this.

Previous versions of CoderRupee came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in CoderRupee with zero setup:
...
You can configure coderrupee to use local models through [Atomic Chat](https://atomic.chat), a desktop application that runs local LLMs behind an OpenAI-compatible API server (default endpoint `http://127.0.0.1:1337/v1`).

```json title="coderrupee.json" "atomic-chat" {5, 6, 8, 10-14}
{
  "$schema": "https://coderrupee.ai/config.json",
  "provider": {
    "atomic-chat": {
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for coderrupee to work properly.
   :::
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX coderrupee
   ```
...
You can use your Azure CLI session instead of an API key. [Install the Azure CLI](https://learn.microsoft.com/en-us/cli/azure/install-azure-cli), run `az login`, then run `/connect`, select **Azure**, and choose **Microsoft Entra ID (Azure CLI)**. CoderRupee lists the Resources visible to your Azure CLI session and their Resource groups. Select a Resource, or choose **Enter another resource name** to enter one manually. If resource listing is unavailable, CoderRupee asks for the name directly. Use `az login --tenant TENANT_ID` if the Resource belongs to a different tenant.
...
CoderRupee finds the Resource group and discovers its deployed models from the active Azure CLI subscription. Run `az account set --subscription NAME_OR_ID` first if the Resource is in a different subscription. Set `AZURE_RESOURCE_GROUP` to skip listing the subscription and query a known Resource directly.

Model discovery requires Azure control-plane permissions, which are separate from inference permissions. If your identity cannot list deployments, CoderRupee keeps the Azure model catalog available instead. Select a model whose name matches your deployment, or configure its deployment name explicitly:
...
Assign your identity the inference role required by the deployment: **Cognitive Services OpenAI User** for Azure OpenAI models or **Cognitive Services User** for other Foundry models. CoderRupee refreshes access tokens through the Azure CLI, including versions earlier than 2.54.0, so you only need to sign in again when the CLI session expires.
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX coderrupee
   ```
...
6. Run the `/models` command to select a model.
   ...
   You can also add models through your coderrupee config.
...
CoderRupee supports two authentication methods:

- **OAuth (Recommended)** — Sign in to your DigitalOcean account; CoderRupee uses your DigitalOcean API token directly for inference and discovers your Inference Routers.
...
3. Your browser opens to authorize CoderRupee. Sign in and approve.

   :::note
   CoderRupee requests `genai:read` and `inference:query` OAuth scopes. Your DigitalOcean API token is used directly for inference — no separate Model Access Key is created.
   :::
...
Inference Routers let you define a routing policy across multiple models — picking the cheapest, fastest, or most appropriate model per request based on the task. After OAuth, CoderRupee surfaces each router as `router:<router-name>` in the model picker.

Selecting a router model is a drop-in replacement for any other model — CoderRupee forwards your request and DigitalOcean picks the underlying model based on your router's policy. Learn more about [Inference Routers](https://docs.digitalocean.com/products/inference/how-to/use-inference-router/)
...
GitLab Duo support in CoderRupee is experimental. Features, configuration, and
behavior may change in future releases.
:::

CoderRupee integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
to store token in coderrupee auth storage.
:::

##### Self-Hosted GitLab

:::note[compliance note]
CoderRupee uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock CoderRupee
to only use your own GitLab-hosted instance, add the following to your
`coderrupee.json` file. It is also recommended to disable session sharing.
...
More documentation on [coderrupee-gitlab-auth](https://www.npmjs.com/package/coderrupee-gitlab-auth) homepage.

##### Configuration

Customize through `coderrupee.json`:
...
When a `duo-workflow-*` model is selected, CoderRupee will:
...
4. Route tool execution requests through CoderRupee's permission-gated tool system
...
{
  "$schema": "https://coderrupee.ai/config.json",
  "plugin": ["coderrupee-gitlab-plugin"]
}
```
...
To use your GitHub Copilot subscription with coderrupee:
...
To use GMI Cloud with CoderRupee:
...
To use Google Vertex AI with CoderRupee:
...
2. Set the required environment variables:
   ...
   Set them while running coderrupee.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id coderrupee
   ```
...
In the event you see a feature or model from Helicone that isn't configured automatically through coderrupee, you can always configure it yourself.
...
```jsonc title="~/.config/coderrupee/coderrupee.jsonc"
{
  "$schema": "https://coderrupee.ai/config.json",
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [coderrupee-helicone-session](https://github.com/H2Shami/coderrupee-helicone-session) plugin to automatically log each CoderRupee conversation as a session in Helicone.

```bash
npm install -g coderrupee-helicone-session
```
...
```json title="coderrupee.json"
{
  "plugin": ["coderrupee-helicone-session"]
}
```

The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each CoderRupee conversation listed as a separate session.
...
You can configure coderrupee to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility

```json title="coderrupee.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://coderrupee.ai/config.json",
  "provider": {
    "llama.cpp": {
...
You can configure coderrupee to use local models through LM Studio.

```json title="coderrupee.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://coderrupee.ai/config.json",
  "provider": {
    "lmstudio": {
...
You can configure coderrupee to use local models through Ollama.

:::tip
Ollama can automatically configure itself for CoderRupee. See the [Ollama integration docs](https://docs.ollama.com/integrations/coderrupee) for details.
:::

```json title="coderrupee.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://coderrupee.ai/config.json",
  "provider": {
    "ollama": {
...
To use Ollama Cloud with CoderRupee:
...
3. Copy the API key for use in CoderRupee.
...
6. **Important**: Before using cloud models in CoderRupee, you must pull the model information locally:
...
CoderRupee Zen is a list of tested and verified models provided by the CoderRupee team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>CoderRupee Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **CoderRupee Zen**.

   ```txt
   /connect
   ```

3. Enter your CoderRupee API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your coderrupee config.
...
5. You can also customize them through your coderrupee config. Here's an example of specifying a provider
...
3. Enter your service key JSON.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' coderrupee
   ```
...
4. Optionally set deployment ID and resource group:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group coderrupee
   ```
...
Don't have a Snowflake account? [Sign up for a free trial](https://signup.snowflake.com/?utm_source=coderrupee&utm_medium=docs&utm_campaign=cortex-provider).

coderrupee's core workflow for coding, editing files, and running commands relies on tool calling. Only the Claude and OpenAI families within Snowflake Cortex support this. The provider is limited to those families to support the core workflow.
...
5. Complete sign-in in the browser that opens. CoderRupee captures the OAuth callback automatically and stores the token — no copy/paste needed.
...
For CI or headless environments, set a PAT or JWT before starting coderrupee:
...
The model catalog is provided automatically. A minimal `coderrupee.json` is all that's needed:
...
You can also customize models through your coderrupee config. Here's an example of specifying provider routing order.
...
2. Select **SuperGrok Subscription**. CoderRupee opens xAI's verification link with the user code pre-populated when supported.

3. Approve the consent screen. If xAI asks for a code, enter the user code displayed by CoderRupee. CoderRupee polls xAI's token endpoint and stores the resulting OAuth tokens once you approve.
...
CoderRupee refreshes the OAuth access token automatically. Any Grok or X Premium plan that includes Grok API access works; you do not need a separate `XAI_API_KEY`.
...
:::tip
You can use any OpenAI-compatible provider with coderrupee. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in coderrupee.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `coderrupee.json` file in your project directory:

   ```json title="coderrupee.json" ""myprovider"" {5-15}
   {
     "$schema": "https://coderrupee.ai/config.json",
     "provider": {
       "myprovider": {
...
```json title="coderrupee.json" {9,11,17-20}
{
  "$schema": "https://coderrupee.ai/config.json",
  "provider": {
    "myprovider": {
...
The `limit` fields allow CoderRupee to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `coderrupee auth list` to see if the credentials
...
2. For custom providers, check the coderrupee config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your coderrupee config.
