---
title: Plugins
description: Écrivez vos propres plugins pour étendre CoderRupee.
---

Les plugins vous permettent d'étendre CoderRupee en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de CoderRupee.
...
- `.coderrupee/plugins/` - Plugins au niveau du projet
- `~/.config/coderrupee/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/coderrupee/node_modules/`.
...
1. Configuration globale (`~/.config/coderrupee/coderrupee.json`)
2. Configuration du projet (`coderrupee.json`)
3. Répertoire global des plugins (`~/.config/coderrupee/plugins/`)
4. Répertoire des plugins du projet (`.coderrupee/plugins/`)
...
CoderRupee exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK coderrupee pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre coderrupee.
...
:::note
Si vous utilisez l'application de bureau CoderRupee, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez coderrupee de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à coderrupee :
...
L'assistant `tool` crée un outil personnalisé que coderrupee peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour coderrupee aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://coderrupee.ai/docs/sdk) pour plus de détails.
