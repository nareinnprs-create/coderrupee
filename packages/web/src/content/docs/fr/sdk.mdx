---
title: SDK
description: Client JS de type sécurisé pour le serveur coderrupee.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK coderrupee JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler coderrupee par programme.
...
Créez une instance de coderrupee :
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `coderrupee.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
console.log(`Server running at ${coderrupee.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de coderrupee, vous pouvez créer une instance client pour vous y connecter :
...
```typescript
import type { Session, Message, Part } from "@coderrupee/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*coderrupee" },
})
