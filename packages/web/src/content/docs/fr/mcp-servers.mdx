...
Vous pouvez ajouter des outils externes à CoderRupee à l'aide du _Model Context Protocol_ ou MCP. CoderRupee prend en charge les serveurs locaux et distants.
...
Vous pouvez définir des serveurs MCP dans votre [CoderRupee Config](https://coderrupee.ai/docs/config/) sous `mcp`. Ajoutez chaque MCP avec un nom unique. Vous pouvez faire référence à ce MCP par son nom lorsque vous demandez le LLM.
...
Les organisations peuvent fournir des serveurs MCP par défaut via leur point de terminaison `.well-known/coderrupee`. Ces serveurs peuvent être désactivés par défaut, permettant aux utilisateurs de choisir ceux dont ils ont besoin.
...
CoderRupee gère automatiquement l'authentification OAuth pour les serveurs MCP distants. Lorsqu'un serveur nécessite une authentification, CoderRupee :
...
Si le serveur nécessite une authentification, CoderRupee vous demandera de vous authentifier lorsque vous essayez de l'utiliser pour la première fois. Sinon, vous pouvez [déclencher manuellement le flux](#authenticating) avec `coderrupee mcp auth <server-name>`.
...
La commande `mcp auth` ouvrira votre navigateur pour autorisation. Après votre autorisation, CoderRupee stockera les jetons en toute sécurité dans `~/.local/share/coderrupee/mcp-auth.json`.
...
Vos MCP sont disponibles sous forme d'outils dans CoderRupee, aux côtés des outils intégrés. Vous pouvez donc les gérer via la configuration CoderRupee comme n'importe quel autre outil.
...
Cela ouvrira une fenêtre de navigateur pour terminer le flux OAuth et connecter CoderRupee à votre compte Sentry.
