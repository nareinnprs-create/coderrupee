...
Pour déboguer les problèmes avec CoderRupee, commencez par vérifier les journaux et les données locales qu'il stocke sur le disque.
...
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\coderrupee\log`
...
Vous pouvez définir le niveau de journalisation avec l'option de ligne de commande `--log-level` pour obtenir des informations de débogage plus détaillées. Par exemple, `coderrupee --log-level DEBUG`.

---

## Stockage

coderrupee stocke les données de session et autres données d'application sur le disque à l'emplacement :

- **macOS/Linux** : `~/.local/share/coderrupee/`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\coderrupee`
...
CoderRupee Desktop exécute un serveur CoderRupee local (le side-car `coderrupee-cli`) en arrière-plan. La plupart des problèmes sont causés par un plugin qui se comporte mal, un cache corrompu ou un mauvais paramètre du serveur.
...
- macOS uniquement : menu `CoderRupee` -> **Recharger la vue Web** (aide si l'interface utilisateur est vide/gelée).
...
- **macOS/Linux** (anciennes installations) : `~/.local/share/coderrupee/coderrupee.jsonc`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\coderrupee\coderrupee.jsonc`
...
CoderRupee peut également charger des plugins locaux à partir du disque. Écartez-les temporairement (ou renommez le dossier) et redémarrez l'application de bureau :

- **Plugins mondiaux**
  - **macOS/Linux** : `~/.config/coderrupee/plugins/`
  - **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\coderrupee\plugins`
...
Si la désactivation des plugins ne résout pas le problème (ou si l'installation d'un plugin est bloquée), videz le cache afin que CoderRupee puisse le reconstruire.

1. Quittez complètement CoderRupee Desktop.
2. Supprimez le répertoire cache :

- **macOS** : Finder -> `Cmd+Shift+G` -> coller `~/.cache/coderrupee`
- **Linux** : supprimez `~/.cache/coderrupee` (ou exécutez `rm -rf ~/.cache/coderrupee`)
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.cache\coderrupee`

3. Redémarrez le bureau CoderRupee.

---

### Résoudre les problèmes de connexion au serveur

CoderRupee Desktop peut soit démarrer son propre serveur local (par défaut), soit se connecter à un serveur URL que vous avez configuré.
...
Si votre `coderrupee.json(c)` contient une section `server`, supprimez-la temporairement et redémarrez l'application de bureau.

#### Vérifier les variables d'environnement

Si `CODERRUPEE_PORT` est défini dans votre environnement, l'application de bureau tentera d'utiliser ce port pour le serveur local.

- Désactivez `CODERRUPEE_PORT` (ou choisissez un port libre) et redémarrez.
...
Sur Windows, CoderRupee Desktop nécessite Microsoft Edge **WebView2 Runtime**. Si l'application s'ouvre sur une fenêtre vide ou ne démarre pas, installez/mettez à jour WebView2 et réessayez.

---

### Windows : problèmes de performances généraux

Si vous rencontrez des performances lentes, des problèmes d'accès aux fichiers ou des problèmes de terminal sur Windows, essayez d'utiliser [WSL (Windows Sous-système pour Linux)](/docs/windows-wsl). WSL fournit un environnement Linux qui fonctionne de manière plus transparente avec les fonctionnalités de CoderRupee.

---

### Les notifications ne s'affichent pas

CoderRupee Desktop affiche uniquement les notifications système lorsque :

- les notifications sont activées pour CoderRupee dans les paramètres de votre système d'exploitation, et
...
1. Quittez le bureau CoderRupee.
2. Recherchez et supprimez ces fichiers (ils se trouvent dans le répertoire de données de l'application CoderRupee Desktop) :

- `coderrupee.settings.dat` (serveur de bureau par défaut URL)
- `coderrupee.global.dat` et `coderrupee.workspace.*.dat` (état de l'interface utilisateur comme les serveurs/projets récents)
...
Si vous rencontrez des problèmes avec CoderRupee :
...
3. Assurez-vous d'avoir la dernière version avec `coderrupee upgrade`
...
Pour déterminer à quels modèles vous avez accès, exécutez `coderrupee models`
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.local\share\coderrupee`
...
Si vous rencontrez des erreurs d’appel API, cela peut être dû à des packages de fournisseurs obsolètes. coderrupee installe dynamiquement les packages du fournisseur (OpenAI, Anthropic, Google, etc.) selon les besoins et les met en cache localement.
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.cache\coderrupee`

2. Redémarrez coderrupee pour réinstaller les derniers packages du fournisseur

Cela forcera coderrupee à télécharger les versions les plus récentes des packages du fournisseur, ce qui résout souvent les problèmes de compatibilité avec les paramètres du modèle et les modifications de API.
...
coderrupee détectera si vous utilisez Wayland et préférez `wl-clipboard`, sinon il essaiera de trouver les outils du presse-papiers dans l'ordre : `xclip` et `xsel`.
