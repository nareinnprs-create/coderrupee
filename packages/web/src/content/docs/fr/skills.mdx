...
Les compétences d'agent permettent à CoderRupee de découvrir des instructions réutilisables à partir de votre dépôt ou de votre répertoire personnel.
...
CoderRupee recherche ces emplacements :

- Configuration du projet : `.coderrupee/skills/<name>/SKILL.md`
- Configuration globale : `~/.config/coderrupee/skills/<name>/SKILL.md`
...
Pour les chemins locaux du projet, CoderRupee remonte de votre répertoire de travail actuel jusqu'à ce qu'il atteigne l'arbre de travail git.
Il charge tout `skills/*/SKILL.md` correspondant dans `.coderrupee/` et tout `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` correspondant en cours de route.

Les définitions globales sont également chargées à partir de `~/.config/coderrupee/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` et `~/.agents/skills/*/SKILL.md`.
...
Créez `.coderrupee/skills/git-release/SKILL.md` comme ceci :
...
CoderRupee répertorie les compétences disponibles dans la description de l'outil `skill`.
...
Contrôlez les compétences auxquelles les agents peuvent accéder à l'aide d'autorisations basées sur des modèles dans `coderrupee.json` :
...
**Pour les agents intégrés** (dans `coderrupee.json`) :
