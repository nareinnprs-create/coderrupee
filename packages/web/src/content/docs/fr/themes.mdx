...
Avec CoderRupee, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, CoderRupee utilise notre propre thème `coderrupee`.
...
CoderRupee est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que CoderRupee corresponde à l'apparence de leur terminal
...
CoderRupee prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/coderrupee/themes/*.json` ou `$XDG_CONFIG_HOME/coderrupee/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.coderrupee/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.coderrupee/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/coderrupee/themes
vim ~/.config/coderrupee/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .coderrupee/themes
vim .coderrupee/themes/my-theme.json
```
...
{
  "$schema": "https://coderrupee.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
