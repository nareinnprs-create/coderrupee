---
title: Serveurs LSP
description: CoderRupee s'intègre à vos serveurs LSP.
---

CoderRupee peut s'intégrer aux serveurs Language Server Protocol (LSP) pour utiliser les diagnostics comme retour pour l'agent.

---

## Serveurs intégrés

CoderRupee est livré avec plusieurs serveurs LSP intégrés pour les langages populaires :
...
:::note
Vous pouvez désactiver les téléchargements automatiques du serveur LSP en définissant la variable d'environnement `CODERRUPEE_DISABLE_LSP_DOWNLOAD` sur `true`.
:::
...
Lorsque LSP est activé et qu'coderrupee ouvre un fichier, il :
...
Vous pouvez activer et personnaliser les serveurs LSP via la section `lsp` de votre configuration coderrupee.
...
```json title="coderrupee.json" {5-7}
{
  "$schema": "https://coderrupee.ai/config.json",
  "lsp": {
    "rust": {
...
```json title="coderrupee.json" {5-9}
{
  "$schema": "https://coderrupee.ai/config.json",
  "lsp": {
    "typescript": {
