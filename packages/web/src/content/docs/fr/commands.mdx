...
Créez `.coderrupee/commands/test.md` :
...
Vous pouvez ajouter des commandes personnalisées via la configuration CoderRupee ou en créant des fichiers markdown dans le répertoire `commands/`.

---

### JSON

Utilisez l'option `command` dans votre CoderRupee [config](/docs/config) :
...
- Par projet : `.coderrupee/commands/`
...
coderrupee comprend plusieurs commandes intégrées comme `/init`, `/undo`, `/redo`, `/share`, `/help` ; [en savoir plus](/docs/tui#commands).
