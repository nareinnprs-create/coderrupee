...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. CoderRupee posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w CoderRupee.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `CODERRUPEE_EXPERIMENTAL_LSP_TOOL=true` (lub `CODERRUPEE_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy CoderRupee lub gdy zmienna środowiskowa `CODERRUPEE_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu CoderRupee:
