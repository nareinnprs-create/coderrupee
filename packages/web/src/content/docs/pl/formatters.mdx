---
title: Formatery
description: CoderRupee używa formaterów specyficznych dla języka.
---

CoderRupee automatycznie formatuje pliki po ich zapisaniu lub edycji przy użyciu formaterów specyficznych dla języka. Zapewnia to, że wygenerowany kod jest zgodny ze stylem kodu Twojego projektu.

---

## Wbudowane

CoderRupee zawiera kilka wbudowanych formaterów dla popularnych języków i frameworków. Poniższa tabela zawiera listę formaterów, obsługiwanych plików oraz wymagań konfiguracyjnych.
...
Jeśli więc Twój projekt zawiera `prettier` w `package.json`, CoderRupee automatycznie go użyje.

---

## Jak to działa

Kiedy CoderRupee zapisuje lub edytuje plik:
...
Możesz dostosować formatery za pomocą sekcji `formatter` w konfiguracji CoderRupee.
