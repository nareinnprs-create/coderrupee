---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w coderrupee.
---

import config from "../../../../config.mjs"
export const console = config.console

coderrupee używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji coderrupee.
...
w `~/.local/share/coderrupee/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim coderrupee
config.
...
CoderRupee Zen to lista modeli dostarczonych przez zespół coderrupee, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z coderrupee. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od CoderRupee Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `CoderRupee Zen` i przejdź do [coderrupee.ai/auth](https://coderrupee.ai/zen).
...
Działa jak każdy inny dostawca w coderrupee i jest całkowicie opcjonalny w użyciu.

---

## CoderRupee Go

CoderRupee Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół coderrupee, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z coderrupee.

1. Uruchom polecenie `/connect` w TUI, wybierz `CoderRupee Go` i przejdź do [coderrupee.ai/auth](https://coderrupee.ai/zen).
...
Aby używać Amazon Bedrock z coderrupee:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania coderrupee:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `coderrupee.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w coderrupee nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować coderrupee do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod coderrupee działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji coderrupee.
...
coderrupee używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować coderrupee
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`coderrupee.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [coderrupee-gitlab-auth](https://www.npmjs.com/package/coderrupee-gitlab-auth).
...
Aby korzystać z subskrypcji GitHub Copilot z kodem coderrupee:
...
Aby używać Google Vertex AI z coderrupee:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania coderrupee.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą coderrupee, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [coderrupee-helicone-session](https://github.com/H2Shami/coderrupee-helicone-session), aby automatycznie rejestrować każdą konwersację coderrupee jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja coderrupee będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować coderrupee tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować coderrupee tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować coderrupee tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem coderrupee. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/coderrupee).
:::
...
Aby korzystać z Ollama Cloud z coderrupee:
...
3. Skopiuj klucz API do użycia w coderrupee.
...
6. **Ważne**: Przed użyciem modeli chmurowych w coderrupee musisz pobrać informacje o modelu lokalnie:
...
CoderRupee Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół coderrupee. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>CoderRupee Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **CoderRupee Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API coderrupee.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji coderrupee.
...
5. Możesz je także dostosować za pomocą konfiguracji coderrupee. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z coderrupee:
...
Możesz także dostosować modele za pomocą konfiguracji coderrupee. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z coderrupee. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `coderrupee.json` w katalogu projektu:
...
Pola `limit` pozwalają coderrupee zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `coderrupee auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację coderrupee i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji coderrupee.
