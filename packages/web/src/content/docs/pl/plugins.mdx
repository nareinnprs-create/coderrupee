---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć coderrupee.
---

Wtyczki umożliwiające wyłączenie coderrupee poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania coderrupee.
...
- `.coderrupee/plugins/` - Wtyczki na poziomie projektu
- `~/.config/coderrupee/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/coderrupee/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/coderrupee/coderrupee.json`)
2. Konfiguracja projektu (`coderrupee.json`)
3. Globalny katalog wtyczek (`~/.config/coderrupee/plugins/`)
4. Katalog wtyczek projektu (`.coderrupee/plugins/`)
...
coderrupee uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z coderrupee do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do coderrupee.
...
:::note
Jeśli korzystasz z aplikacji komputerowej coderrupee, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj coderrupee czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do coderrupee:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod coderrupee. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://coderrupee.ai/docs/sdk).
