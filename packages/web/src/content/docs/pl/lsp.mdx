---
title: Serwery LSP
description: CoderRupee integruje się z serwerami LSP.
---

CoderRupee może integrować się z serwerami Language Server Protocol (LSP), aby używać diagnostyki jako informacji zwrotnej dla agenta.

---

## Wbudowane

CoderRupee posiada kilka wbudowanych serwerów LSP dla następujących języków:
...
:::note
Możesz wyłączyć automatyczne pobieranie serwerów LSP, ustawiając zmienną środowiskową `CODERRUPEE_DISABLE_LSP_DOWNLOAD` na `true`.
:::
...
Gdy LSP jest włączony i CoderRupee otwiera plik:
...
Możesz włączać i dostosowywać serwery LSP poprzez sekcję `lsp` w konfiguracji CoderRupee.
