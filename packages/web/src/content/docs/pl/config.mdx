---
title: Konfiguracja
description: Ustawienia CoderRupee w pliku JSON.
---

Możesz dostosować CoderRupee za pomocą pliku konfiguracyjnego JSON.

---

## Format

CoderRupee obsługuje formaty **JSON** i **JSONC** (JSON z komentarzami).
...
Na przykład, jeśli konfiguracja globalna ustawi `theme: "coderrupee"` i `autoupdate: true`, a konfiguracja projektu ustawi `model: "anthropic/claude-sonnet-4-5"`, ostateczna konfiguracja będzie zawierać wszystkie trzy ustawienia.
...
1. **Konfiguracja zdalna** (z `.well-known/coderrupee`) – ustawienia wymuszane przez organizację
2. **Konfiguracja globalna** (`~/.config/coderrupee/coderrupee.json`) – preferencje użytkownika
3. **Konfiguracja niestandardowa** (zmienna `CODERRUPEE_CONFIG`) — jawne nadpisanie
4. **Konfiguracja projektu** (`coderrupee.json` w projekcie) - ustawienia specyficzne dla projektu
5. **Katalogi `.coderrupee`** - agenci, polecenia, umiejętności
6. **Konfiguracja wbudowana** (zmienna `CODERRUPEE_CONFIG_CONTENT`) — nadpisanie środowiska uruchomieniowego
...
:::note
W katalogach `.coderrupee` i `~/.config/coderrupee` używane są **nazwy w liczbie mnogiej** dla podkatalogów: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Nazwy w liczbie pojedynczej (np. `agent/`) są również obsługiwane dla kompatybilności wstecznej.
:::
...
Organizacje mogą publikować konfigurację pod adresem `.well-known/coderrupee`. Jest ona pobierana automatycznie po uwierzytelnieniu u dostawcy, który ją obsługuje.
...
Umieść swoją globalną konfigurację CoderRupee w `~/.config/coderrupee/coderrupee.json`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak dostawcy, modele i uprawnienia.

Dla ustawień specyficznych dla TUI, użyj `~/.config/coderrupee/tui.json`.
...
Dodaj `coderrupee.json` w katalogu głównym projektu. Konfiguracja projektu ma najwyższy priorytet wśród plików konfiguracyjnych — nadpisuje konfiguracje globalne i zdalne.
...
Kiedy CoderRupee się uruchamia, szuka pliku konfiguracyjnego w katalogu głównym repozytorium Git.
...
Możesz załadować niestandardowy plik konfiguracyjny, ustawiając zmienną środowiskową `CODERRUPEE_CONFIG`.
...
Możesz ustawić niestandardowy katalog konfiguracyjny za pomocą zmiennej środowiskowej `CODERRUPEE_CONFIG_DIR`. Katalog ten będzie przeszukiwany pod kątem agentów, poleceń, trybów i wtyczek, tak jak standardowy katalog `.coderrupee` i powinien zachować tę samą strukturę.
...
Katalog niestandardowy jest ładowany po globalnej konfiguracji i katalogach `.coderrupee`, więc **może być łączony** z ich zawartością.

---

## Schemat

Plik konfiguracyjny ma schemat JSON dostępny pod adresem [**`coderrupee.ai/config.json`**](https://coderrupee.ai/config.json).
...
Użyj `CODERRUPEE_TUI_CONFIG`, aby wskazać niestandardowy plik konfiguracyjny TUI.

Przestarzałe klucze `theme`, `keybinds` i `tui` w `coderrupee.json` są wycofywane i automatycznie migrowane, gdy to możliwe.
...
Możesz skonfigurować serwer dla `coderrupee serve` i `coderrupee web` za pomocą opcji `server`.
...
- `mdns` - Włącz wykrywanie usług mDNS. Dzięki temu inne urządzenia w sieci mogą łatwo znaleźć Twój serwer CoderRupee.
- `mdnsDomain` - Niestandardowa nazwa domeny dla usług mDNS. Domyślnie `coderrupee.local`. Przydatne do uruchamiania wielu instancji w tej samej sieci.
...
Opcja `small_model` konfiguruje oddzielny model do lżejszych zadań, takich jak generowanie tytułów. Domyślnie CoderRupee wybiera tańszy model od tego samego dostawcy, jeśli jest dostępny, w przeciwnym razie powraca do modelu głównego.
...
Możesz także definiować agentów przy użyciu plików markdown w `~/.config/coderrupee/agents/` lub `.coderrupee/agents/`. [Dowiedz się więcej o agentach](/docs/agents).
...
Agent domyślny musi być agentem typu `primary` (nie `subagent`). Może to być agent wbudowany, taki jak `"build"` lub `"plan"`, albo zdefiniowany przez Ciebie [agent niestandardowy](/docs/agents). Jeśli agent nie istnieje lub jest subagentem, CoderRupee powróci do `"build"` z ostrzeżeniem.

Dotyczy to wszystkich interfejsów: TUI, CLI (`coderrupee run`), aplikacji desktopowej i GitHub Actions.
...
Możesz także definiować polecenia przy użyciu plików Markdown w `~/.config/coderrupee/commands/` lub `.coderrupee/commands/`. [Dowiedz się więcej o poleceniach](/docs/commands).
...
CoderRupee może automatycznie pobierać nowe wersje. Możesz to kontrolować za pomocą opcji `autoupdate`.
...
Działa to tylko wtedy, gdy CoderRupee nie został zainstalowany przez menedżera pakietów, takiego jak Homebrew.
...
Domyślnie CoderRupee **zezwala na wszystkie działania** bez konieczności zatwierdzania. Możesz to zmienić za pomocą opcji `permission`.
...
[Wtyczki](/docs/plugins) rozszerzają CoderRupee o niestandardowe narzędzia, hooki i integracje.

Umieść pliki wtyczek w `.coderrupee/plugins/` lub `~/.config/coderrupee/plugins/`. Możesz także załadować wtyczkę z npm za pomocą opcji `plugin`.
...
Jest to przydatne, gdy chcesz ograniczyć użycie CoderRupee tylko do określonych dostawców.
