...
Narzędzia specjalistyczne to funkcje, które tworzysz i które LLM może wywoływać podczas rozmów. Współpracują one z [wbudowanymi narzędziami](/docs/tools) coderrupee, takimi jak `read`, `write` i `bash`.
...
- Lokalnie, umieszczając je w katalogu `.coderrupee/tools/` swojego projektu.
- Lub globalnie, umieszczając je w `~/.config/coderrupee/tools/`.
