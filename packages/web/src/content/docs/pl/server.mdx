---
title: Serwer
description: Interakcja z serwerem coderrupee za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `coderrupee serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient coderrupee.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `coderrupee.local` |
...
Ustaw `CODERRUPEE_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `coderrupee` lub ustaw `CODERRUPEE_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `coderrupee serve`, jak i `coderrupee web`.
...
Kiedy uruchomisz `coderrupee`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera coderrupee do programowej interakcji z coderrupee.
:::

Ta architektura umożliwia obsługę wielu klientów przez coderrupee i programową interakcję z kodem otwartym.

Możesz uruchomić `coderrupee serve`, aby uruchomić samodzielny serwer. Jeśli masz
coderrupee TUI działa, `coderrupee serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki coderrupee [IDE](/docs/ide).
...
Serwer coderrupee udostępnia następujące interfejsy API.
