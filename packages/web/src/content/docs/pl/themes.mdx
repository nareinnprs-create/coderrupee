...
Dzięki coderrupee możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie coderrupee używa naszego własnego motywu `coderrupee`.
...
coderrupee ma kilka wbudowanych motywów.
...
- Chcą, aby CoderRupee pasował do wyglądu ich terminala
...
coderrupee obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/coderrupee/themes/*.json` lub `$XDG_CONFIG_HOME/coderrupee/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.coderrupee/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.coderrupee/themes/*.json`
