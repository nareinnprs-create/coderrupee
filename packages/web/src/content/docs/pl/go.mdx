...
CoderRupee Go to niskokosztowa subskrypcja za **10 $/miesiąc**, która zapewnia niezawodny dostęp do popularnych otwartych modeli do kodowania.

Go działa jak każdy inny dostawca w CoderRupee. Subskrybujesz CoderRupee Go i
...
CoderRupee Go daje Ci dostęp do tych modeli za **10 $/miesiąc**.

---

## Jak to działa

CoderRupee Go działa jak każdy inny dostawca w CoderRupee.

1. Logujesz się do **<a href={console}>CoderRupee Zen</a>**, subskrybujesz Go i
   kopiujesz swój klucz API.
2. Uruchamiasz komendę `/connect` w TUI, wybierasz `CoderRupee Go` i wklejasz
   swój klucz API.
3. Uruchom `/models` w TUI, aby zobaczyć listę modeli dostępnych w ramach Go.

:::note
Tylko jeden członek na obszar roboczy (workspace) może zasubskrybować CoderRupee Go.
:::
...
CoderRupee Go zawiera następujące limity:
...
[ID modelu](/docs/config/#models) w Twojej konfiguracji CoderRupee
używa formatu `coderrupee-go/<model-id>`. Na przykład dla Kimi K3 należy użyć
`coderrupee-go/kimi-k3` w swojej konfiguracji.
...
Stworzyliśmy CoderRupee Go, aby:
...
4. Zapewnić **brak ograniczeń (no lock-in)**, pozwalając na korzystanie z dowolnego innego dostawcy wraz z CoderRupee.
