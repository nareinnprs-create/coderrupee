...
Umiejętności agenta pozwalają coderrupee odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
coderrupee przeszukuje te lokalizacje:

- Project config: `.coderrupee/skills/<name>/SKILL.md`
- Global config: `~/.config/coderrupee/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu coderrupee przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.coderrupee/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/coderrupee/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.coderrupee/skills/git-release/SKILL.md` w ten sposób:
...
coderrupee wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `coderrupee.json`:
...
**Dla agentów wbudowanych** (w `coderrupee.json`):
