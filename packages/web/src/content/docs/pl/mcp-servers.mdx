...
Możesz dodać dodatkowe informacje do coderrupee za pomocą _Model Context Protocol_, w skrócie MCP. coderrupee obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [coderrupee Config](https://coderrupee.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/coderrupee`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
coderrupee automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, coderrupee:
...
Jeśli serwer wymaga uwierzytelnienia, coderrupee poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `coderrupee mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji coderrupee bezpieczne przechowa tokeny w `~/.local/share/coderrupee/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w coderrupee, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie coderrupee, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć coderrupee z kontem Sentry.
