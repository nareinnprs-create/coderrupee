...
يمكنك إضافة أدوات خارجية إلى CoderRupee باستخدام _Model Context Protocol_، أو MCP. يدعم CoderRupee خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات CoderRupee](https://coderrupee.ai/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يمكن للمنظمات توفير خوادم MCP الافتراضية عبر نقطة النهاية `.well-known/coderrupee` الخاصة بها. قد تكون هذه الخوادم معطّلة افتراضيا، مما يتيح للمستخدمين تفعيل ما يحتاجونه فقط.
...
يتولى CoderRupee تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم CoderRupee بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك CoderRupee المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `coderrupee mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن CoderRupee الرموز بشكل آمن في `~/.local/share/coderrupee/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل CoderRupee إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات CoderRupee مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط CoderRupee بحساب Sentry الخاص بك.
