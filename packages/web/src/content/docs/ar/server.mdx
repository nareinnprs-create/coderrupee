---
title: الخادم
description: تفاعل مع خادم coderrupee عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `coderrupee serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل coderrupee استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `coderrupee.local` |
...
عيّن `CODERRUPEE_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `coderrupee`، أو عيّن `CODERRUPEE_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `coderrupee serve` و `coderrupee web`.
...
عند تشغيل `coderrupee` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم coderrupee للتفاعل مع coderrupee برمجيا.
:::

تتيح هذه البنية لـ coderrupee دعم عدة عملاء وتمكّنك من التفاعل مع coderrupee برمجيا.

يمكنك تشغيل `coderrupee serve` لبدء خادم مستقل. إذا كانت واجهة coderrupee في terminal (TUI)
قيد التشغيل، فسيبدأ `coderrupee serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات CoderRupee لـ [IDE](/docs/ide).
...
يعرض خادم coderrupee واجهات API التالية.
