...
تتيح مهارات الوكيل لـ CoderRupee اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث CoderRupee في هذه المواقع:

- إعدادات المشروع: `.coderrupee/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/coderrupee/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج CoderRupee صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.coderrupee/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/coderrupee/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.coderrupee/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض CoderRupee المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `coderrupee.json`:
...
**للوكلاء المدمجين** (في `coderrupee.json`):
