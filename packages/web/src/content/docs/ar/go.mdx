...
CoderRupee Go هو اشتراك منخفض التكلفة بقيمة **$10/شهريًا** يمنحك وصولًا موثوقًا إلى نماذج البرمجة المفتوحة الشائعة.

يعمل Go مثل أي مزود آخر في CoderRupee. تشترك في CoderRupee Go وتحصل على مفتاح API الخاص بك. وهو **اختياري تمامًا**، ولا تحتاج إلى استخدامه لاستخدام CoderRupee.
...
:::tip
اختبرنا مجموعة مختارة من النماذج والمزودين الذين يعملون جيدًا مع CoderRupee.
:::
...
يمنحك CoderRupee Go الوصول إلى هذه النماذج مقابل **$10/شهريًا**.

---

## كيف يعمل

يعمل CoderRupee Go مثل أي مزود آخر في CoderRupee.

1. تسجّل الدخول إلى **<a href={console}>CoderRupee Zen</a>**، وتشترك في Go، ثم تنسخ مفتاح API الخاص بك.
2. تشغّل الأمر `/connect` في TUI، وتختار `CoderRupee Go`، ثم تلصق مفتاح API الخاص بك.
3. شغّل `/models` في TUI لرؤية قائمة النماذج المتاحة عبر Go.

:::note
يمكن لعضو واحد فقط في كل workspace الاشتراك في CoderRupee Go.
:::
...
يتضمن CoderRupee Go الحدود التالية:
...
يستخدم [model id](/docs/config/#models) في إعدادات CoderRupee لديك التنسيق `coderrupee-go/<model-id>`. على سبيل المثال، بالنسبة إلى Kimi K3، ستستخدم `coderrupee-go/kimi-k3` في إعداداتك.
...
4. **عدم فرض قيود ارتباط** من خلال السماح لك أيضًا باستخدام أي مزود آخر مع CoderRupee.
