---
title: المزوّدون
description: استخدام أي موفّر LLM في CoderRupee.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم CoderRupee ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات CoderRupee.
...
في `~/.local/share/coderrupee/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات CoderRupee.
...
CoderRupee Zen هي قائمة نماذج يوفّرها فريق CoderRupee وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع CoderRupee. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع CoderRupee Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر coderrupee، ثم انتقل إلى [coderrupee.ai/auth](https://coderrupee.ai/auth).
...
يعمل مثل أي مزوّد آخر في CoderRupee واستخدامه اختياري بالكامل.

---

## CoderRupee Go

CoderRupee Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق CoderRupee، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع CoderRupee.

1. شغّل الأمر `/connect` في TUI، واختر `CoderRupee Go`، ثم انتقل إلى [coderrupee.ai/auth](https://coderrupee.ai/zen).
...
لاستخدام Amazon Bedrock مع CoderRupee:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل coderrupee:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `coderrupee.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل CoderRupee غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين coderrupee لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل coderrupee بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات coderrupee.
...
تخزين الرمز في مساحة تخزين مصادقة coderrupee.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم CoderRupee نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر CoderRupee
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`coderrupee.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [coderrupee-gitlab-auth](https://www.npmjs.com/package/coderrupee-gitlab-auth).

##### التهيئة

خصّص عبر `coderrupee.json`:
...
لاستخدام اشتراك GitHub Copilot مع coderrupee:
...
لاستخدام Google Vertex AI مع CoderRupee:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر coderrupee، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [coderrupee-helicone-session](https://github.com/H2Shami/coderrupee-helicone-session) لتسجيل كل محادثة CoderRupee تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة CoderRupee مدرجة كجلسة مستقلة.
...
يمكنك ضبط coderrupee لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط coderrupee لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط coderrupee لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع CoderRupee. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/coderrupee) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع CoderRupee:
...
3. انسخ مفتاح API لاستخدامه في CoderRupee.
...
6. **مهم**: قبل استخدام نماذج السحابة في CoderRupee، يجب سحب معلومات النموذج محليا:
...
CoderRupee Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق CoderRupee. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>CoderRupee Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **CoderRupee Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ CoderRupee.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات coderrupee.
...
5. يمكنك أيضا تخصيصها عبر إعدادات coderrupee. إليك مثالا على تحديد مزوّد
...
يمكنك أيضا تخصيص النماذج عبر إعدادات coderrupee. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع coderrupee. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `coderrupee.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ CoderRupee بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `coderrupee auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات coderrupee وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات coderrupee.
