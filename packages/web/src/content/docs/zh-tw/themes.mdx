...
透過 CoderRupee，您可以從多個內建主題中進行選擇，使用能自動適配終端機主題的主題，或者定義您自己的自訂主題。

預設情況下，CoderRupee 使用我們自己的 `coderrupee` 主題。
...
CoderRupee 支援靈活的基於 JSON 的主題系統，讓使用者可以輕鬆建立和自訂主題。
...
1. **內建主題** — 嵌入在二進位檔案中
2. **使用者設定目錄** — 定義在 `~/.config/coderrupee/themes/*.json` 或 `$XDG_CONFIG_HOME/coderrupee/themes/*.json`
3. **專案根目錄** — 定義在 `<project-root>/.coderrupee/themes/*.json`
4. **當前工作目錄** — 定義在 `./.coderrupee/themes/*.json`
