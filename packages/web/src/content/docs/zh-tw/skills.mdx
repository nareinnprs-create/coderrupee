...
代理技能讓 CoderRupee 能夠從你的儲存庫或主目錄中發現可重複使用的指令。
...
- 專案設定：`.coderrupee/skills/<name>/SKILL.md`
- 全域設定：`~/.config/coderrupee/skills/<name>/SKILL.md`
...
對於專案本地路徑，CoderRupee 會從當前工作目錄向上遍歷，直到到達 git 工作樹根目錄。
在此過程中，它會載入 `.coderrupee/` 中所有匹配的 `skills/*/SKILL.md`，以及匹配的 `.claude/skills/*/SKILL.md` 或 `.agents/skills/*/SKILL.md`。

全域定義也會從 `~/.config/coderrupee/skills/*/SKILL.md`、`~/.claude/skills/*/SKILL.md` 和 `~/.agents/skills/*/SKILL.md` 中載入。
...
建立 `.coderrupee/skills/git-release/SKILL.md`，內容如下：
...
CoderRupee 會在 `skill` 工具描述中列出可用技能。
每個條目包含技能名稱和描述：
...
在 `coderrupee.json` 中使用基於模式的權限來控制代理可以存取哪些技能：
