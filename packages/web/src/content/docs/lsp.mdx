---
title: LSP Servers
description: CoderRupee integrates with your LSP servers.
---

CoderRupee can integrate with Language Server Protocol (LSP) servers to use diagnostics as feedback for the agent.

---

## Built-in

CoderRupee comes with several built-in LSP servers for popular languages:
...
:::note
You can disable automatic LSP server downloads by setting the `CODERRUPEE_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
When LSP is enabled and coderrupee opens a file, it:
...
You can enable and customize LSP servers through the `lsp` section in your coderrupee config.
...
```json title="coderrupee.json" {5-8}
{
  "$schema": "https://coderrupee.ai/config.json",
  "lsp": {
    "rust": {
...
```json title="coderrupee.json" {5-13}
{
  "$schema": "https://coderrupee.ai/config.json",
  "lsp": {
    "custom-lsp": {
