---
title: Server
description: Interager med coderrupee-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `coderrupee serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en coderrupee-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `coderrupee.local` |
...
Indstil `CODERRUPEE_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `coderrupee`, eller indstil `CODERRUPEE_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `coderrupee serve` og `coderrupee web`.
...
Når du kører `coderrupee` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug coderrupee-serveren til at interagere med coderrupee programmatisk.
:::

Denne arkitektur lader coderrupee understøtte flere klienter og lader dig interagere med coderrupee programmatisk.

Du kan køre `coderrupee serve` for at starte en selvstændig server. Hvis du har
coderrupee TUI kørende, vil `coderrupee serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af CoderRupee [IDE](/docs/ide) plugins.
...
CoderRupee-serveren viser følgende API'er.
