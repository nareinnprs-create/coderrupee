---
title: Plugins
description: Skriv dine egne plugins for at udvide CoderRupee.
---

Plugins giver dig mulighed for at udvide CoderRupee ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre CoderRupees standardadfærd.
...
- `.coderrupee/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/coderrupee/node_modules/`.
...
1. Global konfiguration (`~/.config/coderrupee/coderrupee.json`)
2. Projektkonfiguration (`coderrupee.json`)
3. Global plugin-mappe (`~/.config/coderrupee/plugins/`)
4. Projekt plugin bibliotek (`.coderrupee/plugins/`)
...
CoderRupee kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En coderrupee SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide coderrupee.
...
:::note
Hvis du bruger CoderRupee desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå coderrupee i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til coderrupee:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som coderrupee kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for coderrupee sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://coderrupee.ai/docs/sdk) for detaljer.
