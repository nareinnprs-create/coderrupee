...
Med CoderRupee kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger CoderRupee vårt eget `coderrupee`-tema.
...
CoderRupee kommer med flere innebygde temaer.
...
- Vil at CoderRupee skal matche terminalens utseende
...
CoderRupee støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/coderrupee/themes/*.json` eller `$XDG_CONFIG_HOME/coderrupee/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.coderrupee/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.coderrupee/themes/*.json`
