---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i CoderRupee.
---

import config from "../../../../config.mjs"
export const console = config.console

CoderRupee bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i CoderRupee-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/coderrupee/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i CoderRupee-konfigurationen.
...
CoderRupee Zen er en liste over modeller leveret af CoderRupee-teamet, som er blevet testet og verificeret til at fungere godt med CoderRupee. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med CoderRupee Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg coderrupee og gå til [coderrupee.ai/auth](https://coderrupee.ai/auth).
...
Det fungerer som alle andre udbydere i CoderRupee og er helt valgfrit at bruge.

---

## CoderRupee Go

CoderRupee Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af CoderRupee-teamet, som er testet og verificeret til at fungere godt med CoderRupee.

1. Kør kommandoen `/connect` i TUI, vælg `CoderRupee Go`, og gå til [coderrupee.ai/auth](https://coderrupee.ai/zen).
...
Sådan bruger du Amazon Bedrock med CoderRupee:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører coderrupee:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `coderrupee.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i CoderRupee understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere coderrupee til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at coderrupee skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem coderrupee-konfigurationen.
...
gemme tokenet i coderrupee auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
CoderRupee bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse CoderRupee
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`coderrupee.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [coderrupee-gitlab-auth](https://www.npmjs.com/package/coderrupee-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `coderrupee.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med CoderRupee:
...
Sådan bruger du Google Vertex AI med CoderRupee:
...
2. Angiv de nødvendige miljøvariabler:
   ...
   Sæt dem mens du kører coderrupee.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem coderrupee, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [coderrupee-helicone-session](https://github.com/H2Shami/coderrupee-helicone-session) plugin for automatisk at logge hver CoderRupee-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver CoderRupee-samtale opført som en separat session.
...
Du kan konfigurere coderrupee til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere coderrupee til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere coderrupee til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for CoderRupee. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/coderrupee) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med CoderRupee:
...
3. Kopier API-nøglen til brug i CoderRupee.
...
6. **Vigtigt**: Før du bruger skymodeller i CoderRupee, skal du hente modelinformationen lokalt:
...
CoderRupee Zen er en liste over testede og verificerede modeller leveret af CoderRupee-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>CoderRupee Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **CoderRupee Zen**.

   ```txt
   /connect
   ```

3. Indtast CoderRupee API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem coderrupee-konfigurationen.
...
5. Du kan også tilpasse dem gennem coderrupee-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med CoderRupee:
...
Du kan også tilpasse modeller gennem coderrupee-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med coderrupee. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `coderrupee.json`-filen i projektkataloget:
...
`limit`-felterne lader CoderRupee forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `coderrupee auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek coderrupee-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i coderrupee-konfigurationen.
