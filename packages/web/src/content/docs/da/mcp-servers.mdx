...
Du kan tilføje eksterne værktøjer til CoderRupee ved hjælp af _Model Context Protocol_ eller MCP. CoderRupee understøtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i din [CoderRupee Config](https://coderrupee.ai/docs/config/) under `mcp`. Tilføj hver MCP med et unikt navn. Du kan henvise til denne MCP ved navn, når du spørger LLM.
...
Organisationer kan levere standard MCP-servere via deres `.well-known/coderrupee`-slutpunkt. Disse servere kan være deaktiveret som standard, hvilket giver brugerne mulighed for at tilmelde sig dem, de har brug for.
...
CoderRupee håndterer automatisk OAuth-godkendelse for eksterne MCP-servere. Når en server kræver godkendelse, vil CoderRupee:
...
Hvis serveren kræver godkendelse, vil CoderRupee bede dig om at godkende, når du prøver at bruge den første gang. Hvis ikke, kan du [manually trigger the flow](#authenticating) med `coderrupee mcp auth <server-name>`.
...
Kommandoen `mcp auth` åbner din browser for godkendelse. Når du har godkendt, vil CoderRupee opbevare tokens sikkert i `~/.local/share/coderrupee/mcp-auth.json`.
...
Dine MCPs er tilgængelige som værktøjer i **TK\_** sammen med indbyggede værktøjer. Så du kan administrere dem gennem CoderRupee-konfigurationen som ethvert andet værktøj.
...
Dette åbner et browservindue for at fuldføre OAuth-flowet og forbinde CoderRupee til din Sentry-konto.
