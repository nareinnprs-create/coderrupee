---
title: Server
description: Komunicirajte s coderrupee serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `coderrupee serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji coderrupee klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `coderrupee.local` |
...
Postavite `CODERRUPEE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `coderrupee`, ili postavite `CODERRUPEE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `coderrupee serve` i za `coderrupee web`.
...
Kada pokrenete `coderrupee`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite coderrupee server za programsku interakciju sa coderrupee-om.
:::

Ova arhitektura omogucava coderrupee podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `coderrupee serve` da startate standalone server. Ako je coderrupee TUI vec pokrenut, `coderrupee serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste CoderRupee [IDE](/docs/ide) pluginovi.
...
coderrupee server izlaže sljedece API-je.
