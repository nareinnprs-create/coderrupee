...
CoderRupee Go je povoljna pretplata od **$10/mjesečno** koja vam pruža pouzdan pristup popularnim otvorenim modelima za programiranje.

Go radi kao bilo koji drugi provajder u CoderRupee-u. Pretplatite se na CoderRupee Go i
dobijete svoj API ključ. On je **potpuno opcionalan** i ne morate ga koristiti da
biste koristili CoderRupee.
...
:::tip
Testirali smo odabranu grupu modela i provajdera koji dobro rade sa CoderRupee-om.
:::
...
CoderRupee Go vam daje pristup ovim modelima za **$10/mjesečno**.

---

## Kako funkcioniše

CoderRupee Go radi kao bilo koji drugi provajder u CoderRupee-u.

1. Prijavite se na **<a href={console}>CoderRupee Zen</a>**, pretplatite se na Go i
   kopirajte svoj API ključ.
2. Pokrenite komandu `/connect` u TUI-ju, odaberite `CoderRupee Go` i zalijepite
   svoj API ključ.
3. Pokrenite `/models` u TUI-ju da vidite listu modela dostupnih kroz Go.

:::note
Samo jedan član po radnom prostoru (workspace) može se pretplatiti na CoderRupee Go.
:::
...
CoderRupee Go uključuje sljedeća ograničenja:
...
[Model id](/docs/config/#models) u vašoj CoderRupee konfiguraciji
koristi format `coderrupee-go/<model-id>`. Na primjer, za Kimi K3, koristili biste
`coderrupee-go/kimi-k3` u svojoj konfiguraciji.
...
Napravili smo CoderRupee Go da bismo:
...
4. Osigurali da **nema zaključavanja (no lock-in)**, omogućavajući vam da uz CoderRupee koristite i bilo kojeg drugog provajdera.
