...
U CoderRupee mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, CoderRupee koristi nasu `coderrupee` temu.
...
CoderRupee dolazi sa vise ugradenih tema.
...
- Zele da CoderRupee odgovara izgledu njihovog terminala
...
CoderRupee podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/coderrupee/themes/*.json` ili `$XDG_CONFIG_HOME/coderrupee/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.coderrupee/themes/*.json`
4. **Trenutni radni direktorij** - `./.coderrupee/themes/*.json`
