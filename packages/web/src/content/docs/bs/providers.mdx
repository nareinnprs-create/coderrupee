---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u CoderRupee.
---

import config from "../../../../config.mjs"
export const console = config.console

CoderRupee koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj CoderRupee konfiguraciji.
...
u `~/.local/share/coderrupee/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem CoderRupee
config.
...
CoderRupee Zen je lista modela koje je obezbedio CoderRupee tim koji su bili
testirano i potvrđeno da dobro radi sa CoderRupee. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa CoderRupee Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite coderrupee i idite na [coderrupee.ai/auth](https://coderrupee.ai/auth).
...
Radi kao i svaki drugi provajder u CoderRupee i potpuno je opcionalan za korištenje.

---

## CoderRupee Go

CoderRupee Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža CoderRupee tim i koji su testirani i verificirani da dobro rade s CoderRupee-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `CoderRupee Go` i idite na [coderrupee.ai/auth](https://coderrupee.ai/zen).
...
Da biste koristili Amazon Bedrock s CoderRupee:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete coderrupee:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `coderrupee.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u CoderRupee.
:::
...
Možete konfigurirati coderrupee za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi coderrupee ispravno radio.
   :::
...
Također možete dodati modele kroz svoju coderrupee konfiguraciju.
...
da pohrani token u coderrupee auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
CoderRupee koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate CoderRupee
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`coderrupee.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [coderrupee-gitlab-auth](https://www.npmjs.com/package/coderrupee-gitlab-auth).

##### Konfiguracija

Prilagodite putem `coderrupee.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s coderrupee:
...
Za korištenje Google Vertex AI s CoderRupee:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće coderrupee.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem coderrupeea, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [coderrupee-helicone-session](https://github.com/H2Shami/coderrupee-helicone-session) da automatski prijavite svaki CoderRupee razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki CoderRupee razgovor naveden kao zasebna sesija.
...
Možete konfigurirati coderrupee za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati coderrupee za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati coderrupee za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za CoderRupee. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/coderrupee) za detalje.
:::
...
Da biste koristili Ollama Cloud s CoderRupee:
...
3. Kopirajte API ključ za korištenje u CoderRupee.
...
6. **Važno**: Prije upotrebe modela oblaka u CoderRupee, morate lokalno povući informacije o modelu:
...
CoderRupee Zen je lista testiranih i verifikovanih modela koju je obezbedio CoderRupee tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>CoderRupee Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **CoderRupee Zen**.

```txt
   /connect
```

3. Unesite svoj CoderRupee API ključ.
...
Također možete dodati dodatne modele putem vaše coderrupee konfiguracije.
...
5. Također ih možete prilagoditi putem vaše coderrupee konfiguracije. Evo primjera navođenja provajdera
...
Također možete prilagoditi modele kroz svoju coderrupee konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s coderrupee-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `coderrupee.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju CoderRupee da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `coderrupee auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite CoderRupee konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj coderrupee konfiguraciji.
