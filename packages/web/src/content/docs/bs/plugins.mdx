---
title: Dodaci
description: Napišite vlastite dodatke za proširenje CoderRupee.
---

Dodaci vam omogućavaju da proširite CoderRupee spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja CoderRupee.
...
- `.coderrupee/plugins/` - Dodaci na nivou projekta
- `~/.config/coderrupee/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/coderrupee/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/coderrupee/coderrupee.json`)
2. Konfiguracija projekta (`coderrupee.json`)
3. Globalni direktorij dodataka (`~/.config/coderrupee/plugins/`)
4. Direktorij dodataka projekta (`.coderrupee/plugins/`)
...
CoderRupee pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje CoderRupee.
...
Spriječite coderrupee da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u coderrupee:
...
Pomoćnik `tool` kreira prilagođeni alat koji coderrupee može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za coderrupee zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://coderrupee.ai/docs/sdk) za detalje.
