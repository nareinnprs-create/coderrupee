---
title: Konfiguracija
description: Korištenje CoderRupee JSON konfiguracije.
---

Možete konfigurirati CoderRupee koristeći JSON konfiguracijski fajl.

---

## Format

CoderRupee podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/coderrupee`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/coderrupee/coderrupee.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`CODERRUPEE_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`coderrupee.json` u projektu) - postavke specifične za projekat
5. **`.coderrupee` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`CODERRUPEE_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.coderrupee` i `~/.config/coderrupee` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/coderrupee` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu CoderRupee konfiguraciju u `~/.config/coderrupee/coderrupee.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/coderrupee/tui.json`.
...
Dodajte `coderrupee.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se CoderRupee pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `CODERRUPEE_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `CODERRUPEE_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.coderrupee` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.coderrupee`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`coderrupee.ai/config.json`**](https://coderrupee.ai/config.json).

TUI konfiguracija koristi [**`coderrupee.ai/tui.json`**](https://coderrupee.ai/tui.json).
...
Koristite `CODERRUPEE_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `coderrupee.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `coderrupee serve` i `coderrupee web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš CoderRupee server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `coderrupee.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj CoderRupee konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, CoderRupee pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/coderrupee/agents/` ili `.coderrupee/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, CoderRupee će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`coderrupee run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/coderrupee/commands/` ili `.coderrupee/commands/`. [Saznajte više ovdje](/docs/commands).
...
CoderRupee će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, CoderRupee **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju CoderRupee sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.coderrupee/plugins/` ili `~/.config/coderrupee/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite CoderRupee da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
