...
Postoje dvije vrste agenata u CoderRupee: primarni agenti i podagenti.
...
CoderRupee dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
CoderRupee dolazi sa tri ugrađena subagenta, **General**, **Explore** i **Scout**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

CoderRupee dolazi sa dva ugrađena primarna agenta i tri ugrađena subagenta.
...
Agent samo za čitanje za istraživanje eksterne dokumentacije i zavisnosti. Koristite ga kada trebate klonirati repozitorij zavisnosti u CoderRupee-ov upravljani cache, pregledati izvorni kod biblioteke ili uporediti lokalni kod sa upstream implementacijama bez mijenjanja vašeg radnog prostora.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `coderrupee.json`:
...
- Globalno: `~/.config/coderrupee/agents/`
- Po projektu: `.coderrupee/agents/`
...
Ako temperatura nije navedena, CoderRupee koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu CoderRupee konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj CoderRupee konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [CoderRupee Zen](/docs/zen), koristili biste `coderrupee/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `coderrupee models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/coderrupee/coderrupee).
:::
