...
Možete dodati vanjske alate u CoderRupee koristeći _Model Context Protocol_, ili MCP. CoderRupee podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [CoderRupee Config](https://coderrupee.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/coderrupee`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
CoderRupee automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, CoderRupee će:
...
Ako server zahtijeva autentifikaciju, CoderRupee će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `coderrupee mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, CoderRupee sigurno čuva tokene u `~/.local/share/coderrupee/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u CoderRupee, zajedno s ugrađenim alatima. Možete njima upravljati kroz CoderRupee konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete CoderRupee sa vašim Sentry nalogom.
