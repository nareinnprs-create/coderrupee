...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. CoderRupee dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u CoderRupee.
...
:::note
Ovaj alat je dostupan samo kada je `CODERRUPEE_EXPERIMENTAL_LSP_TOOL=true` (ili `CODERRUPEE_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz CoderRupee provajdera ili kada je varijabla `CODERRUPEE_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju CoderRupee:
