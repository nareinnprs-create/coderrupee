...
Da biste otklonili probleme s CoderRupee, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\coderrupee\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `coderrupee --log-level DEBUG`.

---

## Pohrana

CoderRupee pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/coderrupee/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\coderrupee`
...
CoderRupee Desktop pokreće lokalni CoderRupee server (`coderrupee-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `CoderRupee` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/coderrupee/coderrupee.jsonc` (ili `~/.config/coderrupee/coderrupee.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/coderrupee/coderrupee.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\coderrupee\coderrupee.jsonc`
...
CoderRupee također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/coderrupee/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\coderrupee\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga CoderRupee mogao ponovo izgraditi.

1. Potpuno zatvorite CoderRupee Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/coderrupee`
- **Linux**: obrišite `~/.cache/coderrupee` (ili pokrenite `rm -rf ~/.cache/coderrupee`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\coderrupee`

3. Ponovo pokrenite CoderRupee Desktop.

---

### Rješavanje problema sa vezom na serveru

CoderRupee Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `coderrupee.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `CODERRUPEE_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `CODERRUPEE_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, CoderRupee Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa CoderRupee karakteristikama.

---

### Obavještenja se ne prikazuju

CoderRupee Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za CoderRupee u postavkama vašeg OS-a, i
...
1. Zatvorite CoderRupee Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka CoderRupee Desktop aplikacije):

- `coderrupee.settings.dat` (URL zadanog servera za desktop)
- `coderrupee.global.dat` i `coderrupee.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Ako imate problema s CoderRupee:
...
Da saznate kojim modelima imate pristup, pokrenite `coderrupee models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\coderrupee`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. CoderRupee dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\coderrupee`

2. Ponovo pokrenite CoderRupee da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti CoderRupee da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
CoderRupee će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
