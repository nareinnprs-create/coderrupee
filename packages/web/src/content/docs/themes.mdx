...
With CoderRupee you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, CoderRupee uses our own `coderrupee` theme.
...
CoderRupee comes with several built-in themes.
...
- Want CoderRupee to match their terminal's appearance
...
CoderRupee supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/coderrupee/themes/*.json` or `$XDG_CONFIG_HOME/coderrupee/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.coderrupee/themes/*.json`
4. **Current working directory** - Defined in `./.coderrupee/themes/*.json`
