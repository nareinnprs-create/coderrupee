...
Create `.coderrupee/commands/test.md`:
...
You can add custom commands through the CoderRupee config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your CoderRupee [config](/docs/config):
...
- Per-project: `.coderrupee/commands/`
...
coderrupee includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
