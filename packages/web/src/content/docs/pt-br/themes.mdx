...
Com o coderrupee, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o coderrupee usa nosso próprio tema `coderrupee`.
...
O coderrupee vem com vários temas embutidos.
...
- Querem que o coderrupee corresponda à aparência do seu terminal
...
O coderrupee suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/coderrupee/themes/*.json` ou `$XDG_CONFIG_HOME/coderrupee/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.coderrupee/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.coderrupee/themes/*.json`
