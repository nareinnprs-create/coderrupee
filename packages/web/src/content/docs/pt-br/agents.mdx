...
Existem dois tipos de agentes no coderrupee; agentes primários e subagentes.
...
coderrupee vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
CoderRupee vem com três subagentes integrados, **General**, **Explore** e **Scout**. Vamos ver isso abaixo.

---

## Integrados

CoderRupee vem com dois agentes primários integrados e três subagentes integrados.
...
Um agente somente leitura para pesquisa em documentação externa e dependências. Use-o quando você precisar clonar o repositório de uma dependência para o cache gerenciado do CoderRupee, inspecionar o código-fonte de uma biblioteca ou cruzar o código local com implementações upstream sem modificar seu workspace.
...
Configure os agentes em seu arquivo de configuração `coderrupee.json`:
...
- Global: `~/.config/coderrupee/agents/`
- Por projeto: `.coderrupee/agents/`

```markdown title="~/.config/coderrupee/agents/review.md"
---
...
Se nenhuma temperatura for especificada, o coderrupee usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do coderrupee quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do coderrupee usa o formato `provider/model-id`. Por exemplo, se você estiver usando [CoderRupee Zen](/docs/zen), você usaria `coderrupee/gpt-5.1-codex` para GPT 5.1 Codex.
...
```json title="coderrupee.json" {3-6,9-12}
{
  "$schema": "https://coderrupee.ai/config.json",
  "tools": {
    "write": true,
...
```json title="coderrupee.json" {3-5,8-10}
{
  "$schema": "https://coderrupee.ai/config.json",
  "permission": {
    "edit": "deny"
...
```json title="coderrupee.json" {6,7}
{
  "agent": {
    "deep-thinker": {
...
:::tip
Execute `coderrupee models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/coderrupee/coderrupee).
:::
...
```markdown title="~/.config/coderrupee/agents/docs-writer.md"
---
...
```markdown title="~/.config/coderrupee/agents/security-auditor.md"
---
