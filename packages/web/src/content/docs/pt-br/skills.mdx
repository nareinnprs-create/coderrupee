...
As habilidades do agente permitem que o coderrupee descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O coderrupee pesquisa nesses locais:

- Configuração do projeto: `.coderrupee/skills/<name>/SKILL.md`
- Configuração global: `~/.config/coderrupee/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o coderrupee sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.coderrupee/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/coderrupee/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.coderrupee/skills/git-release/SKILL.md` assim:
...
O coderrupee lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `coderrupee.json`:
...
**Para agentes embutidos** (em `coderrupee.json`):
