---
title: Servidor
description: Interaja com o servidor coderrupee via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `coderrupee serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente coderrupee pode usar.
...
```bash
coderrupee serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `coderrupee.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
coderrupee serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `CODERRUPEE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `coderrupee`, ou defina `CODERRUPEE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `coderrupee serve` quanto ao `coderrupee web`.

```bash
CODERRUPEE_SERVER_PASSWORD=your-password coderrupee serve
```
...
Quando você executa `coderrupee`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor coderrupee para interagir com o coderrupee programaticamente.
:::

Essa arquitetura permite que o coderrupee suporte múltiplos clientes e permite que você interaja com o coderrupee programaticamente.

Você pode executar `coderrupee serve` para iniciar um servidor autônomo. Se você tiver o TUI do coderrupee em execução, `coderrupee serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do coderrupee [IDE](/docs/ide).
...
O servidor coderrupee expõe as seguintes APIs.
