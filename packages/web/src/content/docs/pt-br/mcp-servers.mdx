...
Você pode adicionar ferramentas externas ao coderrupee usando o _Model Context Protocol_, ou MCP. O coderrupee suporta servidores locais e remotos.
...
Você pode definir servidores MCP em sua [Configuração do coderrupee](https://coderrupee.ai/docs/config/) sob `mcp`. Adicione cada MCP com um nome único. Você pode se referir a esse MCP pelo nome ao solicitar ao LLM.
...
As organizações podem fornecer servidores MCP padrão através de seu endpoint `.well-known/coderrupee`. Esses servidores podem estar desabilitados por padrão, permitindo que os usuários optem pelos que precisam.
...
O coderrupee lida automaticamente com a autenticação OAuth para servidores MCP remotos. Quando um servidor requer autenticação, o coderrupee irá:
...
Se o servidor requer autenticação, o coderrupee solicitará que você se autentique quando tentar usá-lo pela primeira vez. Se não, você pode [iniciar manualmente o fluxo](#authenticating) com `coderrupee mcp auth <server-name>`.
...
O comando `mcp auth` abrirá seu navegador para autorização. Após você autorizar, o coderrupee armazenará os tokens de forma segura em `~/.local/share/coderrupee/mcp-auth.json`.
...
Seus MCPs estão disponíveis como ferramentas no coderrupee, juntamente com ferramentas integradas. Portanto, você pode gerenciá-los através da configuração do coderrupee como qualquer outra ferramenta.
...
Isso abrirá uma janela do navegador para completar o fluxo OAuth e conectar o coderrupee à sua conta do Sentry.
