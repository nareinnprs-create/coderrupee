---
title: Plugins
description: Escreva seus próprios plugins para estender o coderrupee.
---

Plugins permitem que você estenda o coderrupee conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do coderrupee.
...
- `.coderrupee/plugins/` - Plugins em nível de projeto
- `~/.config/coderrupee/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/coderrupee/node_modules/`.
...
1. Configuração global (`~/.config/coderrupee/coderrupee.json`)
2. Configuração do projeto (`coderrupee.json`)
3. Diretório de plugins global (`~/.config/coderrupee/plugins/`)
4. Diretório de plugins do projeto (`.coderrupee/plugins/`)
...
O coderrupee executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do coderrupee para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o coderrupee.
...
:::note
Se você estiver usando o aplicativo desktop coderrupee, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o coderrupee de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao coderrupee:
...
O helper `tool` cria uma ferramenta personalizada que o coderrupee pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o coderrupee junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://coderrupee.ai/docs/sdk) para detalhes.
