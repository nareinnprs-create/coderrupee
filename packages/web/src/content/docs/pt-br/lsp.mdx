...
description: O coderrupee integra-se aos seus servidores LSP.
---

O coderrupee pode se integrar a servidores Language Server Protocol (LSP) para usar diagnósticos como feedback para o agente.

---

## Integrados

O coderrupee vem com vários servidores LSP integrados para linguagens populares:
...
:::note
Você pode desabilitar os downloads automáticos do servidor LSP definindo a variável de ambiente `CODERRUPEE_DISABLE_LSP_DOWNLOAD` como `true`.
:::
...
Quando o LSP está habilitado e o coderrupee abre um arquivo, ele:
...
Você pode habilitar e personalizar os servidores LSP através da seção `lsp` na sua configuração do coderrupee.
