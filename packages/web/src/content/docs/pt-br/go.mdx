...
O CoderRupee Go é uma assinatura de baixo custo de **US$ 10/mês** que oferece acesso confiável a modelos abertos de programação populares.

O Go funciona como qualquer outro provedor no CoderRupee. Você assina o CoderRupee Go e
...
O CoderRupee Go lhe dá acesso a esses modelos por **US$ 10/mês**.

---

## Como funciona

O CoderRupee Go funciona como qualquer outro provedor no CoderRupee.

1. Você entra no **<a href={console}>CoderRupee Zen</a>**, assina o Go e
   copia a sua chave de API.
2. Você executa o comando `/connect` na TUI, seleciona `CoderRupee Go` e cola
   a sua chave de API.
3. Execute `/models` na TUI para ver a lista de modelos disponíveis através do Go.

:::note
Apenas um membro por workspace pode assinar o CoderRupee Go.
:::
...
O CoderRupee Go inclui os seguintes limites:
...
O [ID do modelo](/docs/config/#models) na sua configuração do CoderRupee
usa o formato `coderrupee-go/<model-id>`. Por exemplo, para o Kimi K3, você usaria
`coderrupee-go/kimi-k3` na sua configuração.
...
4. Não ter **nenhuma restrição de fornecedor (lock-in)**, permitindo que você use qualquer outro provedor com o CoderRupee também.
