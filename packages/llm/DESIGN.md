...
> current private `@coderrupee/llm` API. Names and exact TypeScript signatures
> are illustrative until implementation, but the domain boundaries and defaults
> are deliberate.

## Status

- Proposed package: `@coderrupee/ai`
...
- Secondary audience: CoderRupee and other durable agent runtimes
...
import { LLM } from "@coderrupee/ai"
import { OpenAI } from "@coderrupee/ai/providers/openai"
...
```ts
import { Message } from "@coderrupee/ai"
...
import { LLM, Tool } from "@coderrupee/ai"
...
CoderRupee and other durable runtimes need to own persistence, tool settlement,
...
```ts
import { LLM } from "@coderrupee/ai/promise"
import { OpenAI } from "@coderrupee/ai/providers/openai"
...
```ts
import { LLMSchema } from "@coderrupee/ai/schema"
...
```ts
import { Provider, Protocol } from "@coderrupee/ai/provider"
...
@coderrupee/ai/promise
  Promise/AsyncIterable LLM facade

@coderrupee/ai/schema
  serializable domain schemas

@coderrupee/ai/provider
  experimental Provider and Protocol authoring APIs

@coderrupee/ai/providers/openai
@coderrupee/ai/providers/anthropic
@coderrupee/ai/providers/google
...
```
...
CoderRupee should migrate to `generateTurn` / `streamTurn`, preserving its durable
