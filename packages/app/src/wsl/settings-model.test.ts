...
describe("WSL server settings presentation", () => {
  ...
  test("offers install and update only when CoderRupee needs attention", () => {
    ...
    expect(
      wslOpencodeAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/coderrupee",
        version: "1.2.2",
        expectedVersion: "1.2.3",
  ...
  test("does not report ready when CoderRupee is present but cannot run", () => {
    const model = addServerViewModel({
      state: {
        ...readyWslState,
        installed: [{ name: "Debian", version: 2, isDefault: true }],
        ...
        coderrupeeChecks: {
          Debian: {
            distro: "Debian",
            resolvedPath: "/home/me/.coderrupee/bin/coderrupee",
            version: null,
            expectedVersion: "1.2.3",
            matchesDesktop: null,
            error: "coderrupee is installed but could not run",
          },
        },
      },
      view: "main",
    ...
    expect(model.primaryButton.action).toBe("install-coderrupee")
  })
