...
export type AddServerPrimaryButton = {
  ...
  label: AddServerText
  disabled: boolean
  action: "install-coderrupee" | "add" | null
  loading: boolean
  width: string | null
}
...
export function addServerViewModel(input: {
  ...
  const coderrupeeCheck = selectedDistro ? (state?.coderrupeeChecks[selectedDistro] ?? null) : null
...
function addServerDistroStatus(input: {
  ...
  const check = input.state?.coderrupeeChecks[input.name]
  if (!check) {
    if (input.probingAddable || (job?.kind === "probe-addable" && job.distros.includes(input.name))) {
      return checkingStatus()
    }
  ...
  if (!check.resolvedPath) return { label: { key: "wsl.onboarding.distroStatus.coderrupeeMissing" }, tone: "warning" }
...
function addServerPrimaryButton(input: {
  state: WslServersState | undefined
  selectedDistro: string | null
  coderrupeeCheck: WslOpencodeCheck | null
  adding: boolean
  probingAddable: boolean
  ...
  const installingOpencode =
    input.state?.job?.kind === "install-coderrupee" && input.state.job.distro === input.selectedDistro
  ...
  if (!addServerOpencodeReady(input.coderrupeeCheck)) {
    const update = !!input.coderrupeeCheck?.resolvedPath && input.coderrupeeCheck.matchesDesktop === false
    return {
      variant: "neutral",
...
function addServerSelectedDistroSettled(state: WslServersState | undefined, selectedDistro: string | null) {
  ...
  return !!state.coderrupeeChecks[selectedDistro]
}
...
export function addableProbePlan(input: {
  ...
  const pending = ordered.flatMap((item) => {
    ...
    if (wslDistroReady(state, item.name) && !state.coderrupeeChecks[item.name]) return [`coderrupee:${item.name}`]
    return []
  })
