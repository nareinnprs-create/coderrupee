...
import type { SessionNotFoundError } from "@coderrupee/sdk/v2/client"
...
describe("parseReadableConfigInvalidError", () => {
  test("formats issues with file path", () => {
    ...
    expect(result).toBe(
      ["Arquivo de config em coderrupee.config.ts invalido: settings.host: Required", "mode: Invalid"].join("\n"),
    )
  })
