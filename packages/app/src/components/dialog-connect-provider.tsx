import type { IntegrationMethod, IntegrationOauthConnectOutput } from "@coderrupee/client/promise"
import { Button } from "@coderrupee/ui/button"
import { useDialog } from "@coderrupee/ui/context/dialog"
import { Dialog } from "@coderrupee/ui/dialog"
import { Icon } from "@coderrupee/ui/icon"
import { IconButton } from "@coderrupee/ui/icon-button"
import { List, type ListRef } from "@coderrupee/ui/list"
import { ProviderIcon } from "@coderrupee/ui/provider-icon"
import { Spinner } from "@coderrupee/ui/spinner"
import { Tag } from "@coderrupee/ui/tag"
import { TextField } from "@coderrupee/ui/text-field"
import { ButtonV2 } from "@coderrupee/ui/v2/button-v2"
import { DialogBody, DialogHeader, DialogTitle, DialogV2 } from "@coderrupee/ui/v2/dialog-v2"
...
function ProviderPicker(props: {
  ...
  const note = (id: string) => {
    ...
    if (id === "coderrupee-go") return language.t("dialog.provider.coderrupeeGo.tagline")
    return undefined
  }
    ...
    >
      {(i) => (
        <div class="px-1.25 w-full flex items-center gap-x-3">
          ...
          <span>{i.name}</span>
          <Show when={i.id === "coderrupee"}>
            <div class="text-14-regular text-text-weak">{language.t("dialog.provider.coderrupee.tagline")}</div>
          </Show>
...
function ProviderPickerV2(props: {
  ...
  const featured = ["coderrupee", "coderrupee-go", "anthropic", "openai", "google", "openrouter", "vercel"]
                  ...
                  <For each={group.items()}>
                    {(provider) => (
                      ...
                      >
                        ...
                        <Show when={provider.id === "coderrupee" || provider.id === "coderrupee-go"}>
                          <span class="min-w-0 truncate font-[440] text-v2-text-text-muted">
                            {language.t(
                              provider.id === "coderrupee"
                                ? "dialog.provider.coderrupee.tagline"
                                : "dialog.provider.coderrupeeGo.tagline",
                            )}
                          </span>
  ...
  function ApiAuthView() {
    ...
    if (newLayout())
      return (
        <div class="flex flex-col gap-5 px-3 text-[13px] font-[440] leading-5 tracking-[-0.04px] text-v2-text-text-muted">
          <Show
            when={provider().id === "coderrupee"}
            fallback={language.t("provider.connect.apiKey.description", { provider: provider().name })}
          >
            <div class="flex flex-col gap-5">
              <div>{language.t("provider.connect.coderrupeeZen.line1")}</div>
              <div>{language.t("provider.connect.coderrupeeZen.line2")}</div>
              <div>
                {language.t("provider.connect.coderrupeeZen.visit.prefix")}
                <ExternalLink
                  href="https://coderrupee.ai/zen"
                  class="text-v2-text-text-base focus-visible:rounded-xs focus-visible:outline-2 focus-visible:outline-v2-border-border-focus"
                >
                  {language.t("provider.connect.coderrupeeZen.visit.link")}
                </ExternalLink>
                {language.t("provider.connect.coderrupeeZen.visit.suffix")}
              </div>
            </div>
          </Show>
    ...
    return (
      <div class="flex flex-col gap-6">
        <Switch>
          <Match when={provider().id === "coderrupee"}>
            <div class="flex flex-col gap-4">
              <div class="text-14-regular text-text-base">{language.t("provider.connect.coderrupeeZen.line1")}</div>
              <div class="text-14-regular text-text-base">{language.t("provider.connect.coderrupeeZen.line2")}</div>
              <div class="text-14-regular text-text-base">
                {language.t("provider.connect.coderrupeeZen.visit.prefix")}
                <ExternalLink href="https://coderrupee.ai/zen" tabIndex={-1}>
                  {language.t("provider.connect.coderrupeeZen.visit.link")}
                </ExternalLink>
                {language.t("provider.connect.coderrupeeZen.visit.suffix")}
              </div>
