...
import type {
  ...
  ReferenceListOutput,
  SessionApi,
} from "@coderrupee/client/promise"
...
import { NormalizedProviderListResponse } from "@coderrupee/session-ui/context"
    ...
    queryFn: () =>
      retry(() =>
        api.list().then((projects) => {
          return projects
            .filter((p) => !!p?.id)
            .filter((p) => !!p.worktree && !p.worktree.includes("coderrupee-test"))
