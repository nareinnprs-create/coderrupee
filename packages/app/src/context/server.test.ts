  ...
  test("moves an existing canonical web bucket into local scope", () => {
    expect(
      migrateCanonicalLocalServerState(
        {
          list: [],
          projects: { "https://coderrupee.example.com": [{ worktree: "/remote", expanded: true }] },
          lastProject: { "https://coderrupee.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://coderrupee.example.com"),
      ),
    ).toEqual({
      list: [],
      ...
      migrateCanonicalLocalServerState(
        {
          projects: {
            ...
            "https://coderrupee.example.com": [
              { worktree: "/local", expanded: true },
              { worktree: "/remote", expanded: true },
            ],
          },
          lastProject: { local: "/local", "https://coderrupee.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://coderrupee.example.com"),
      ),
    ).toEqual({
