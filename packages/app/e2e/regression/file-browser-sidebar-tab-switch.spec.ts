...
const directory = "C:/CoderRupee/FileBrowserSidebar"
...
async function setup(
  ...
  await mockCoderRupeeServer(page, {
    ...
    provider: {
      ...
      default: { providerID: "coderrupee", modelID: "test" },
    },
    sessions: [
      {
        id: sessionID,
  ...
  await page.addInitScript(
    ({ directory, server, sessionID }) => {
      ...
      localStorage.setItem(
        "coderrupee.global.dat:layout",
        JSON.stringify({ review: { diffStyle: "split", panelOpened: true } }),
      )
      localStorage.setItem(
        "coderrupee.global.dat:review-panel-v2",
        JSON.stringify({ sidebarOpened: true, sidebarWidth: 240, expandMode: "collapse" }),
      )
      localStorage.setItem(
        "coderrupee.window.browser.dat:tabs",
