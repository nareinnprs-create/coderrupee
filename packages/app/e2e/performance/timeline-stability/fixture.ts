...
import { SessionStatusEvent } from "@coderrupee/schema/session-status-event"
import { SessionV1 } from "@coderrupee/schema/session-v1"
import type {
  AssistantMessage,
  GlobalEvent,
  Message,
...
export const directory = "C:/CoderRupee/TimelineStability"
...
export const model = { providerID: "coderrupee", modelID: "claude-opus-4-6", variant: "max" }
...
export async function setupTimeline(
  ...
  if (input.locale) {
    await page.addInitScript((locale) => {
      localStorage.setItem("coderrupee.global.dat:language", JSON.stringify({ locale }))
    }, input.locale)
  }
