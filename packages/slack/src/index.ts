...
import { createOpencode, type ToolPart } from "@coderrupee/sdk"
...
console.log("🚀 Starting coderrupee server...")
const coderrupee = await createOpencode({
  port: 0,
})
...
void (async () => {
  const events = await coderrupee.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new coderrupee session...")
    const { client, server } = coderrupee
    ...
    console.log("✅ Created coderrupee session:", createResult.data.id)
  ...
  console.log("📝 Sending to coderrupee:", message.text)
