...
Delete `packages/coderrupee/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/coderrupee/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/coderrupee/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/coderrupee`
- TUI plugin loader tests in `packages/coderrupee` if runtime plugin API wiring changed
