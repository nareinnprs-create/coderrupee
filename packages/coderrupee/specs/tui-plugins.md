...
- Author package entrypoint is `@coderrupee/plugin/tui`.
...
```json
{
  "$schema": "https://coderrupee.ai/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
  ...
  "plugin": ["@acme/coderrupee-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@coderrupee/plugin/tui`.
- `@coderrupee/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@coderrupee/plugin/tui"
...
- The value is a semver range checked against the running CoderRupee version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.coderrupee` is absent, no check is performed (backward compatible).
...
- `coderrupee plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `coderrupee plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.coderrupee` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.coderrupee`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.coderrupee`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.coderrupee` dir or `CODERRUPEE_CONFIG_DIR`, CoderRupee installs `@coderrupee/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@coderrupee/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus CoderRupee's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
CoderRupee registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, CoderRupee pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@coderrupee/plugin/tui`.
...
- `message` is required; `title` defaults to `"coderrupee"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `coderrupee.default`.
...
- Local plugins persist installed themes under the local `.coderrupee/themes` area near the plugin config source.
...
- `--pure` / `CODERRUPEE_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.coderrupee/plugins/tui-smoke.tsx`
- Local vim plugin: `.coderrupee/plugins/tui-vim.tsx`
- Local smoke config: `.coderrupee/tui.json`
- Local smoke theme: `.coderrupee/plugins/smoke-theme.json`
