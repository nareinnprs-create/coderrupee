...
Practical reference for a future `packages/server` split after the coderrupee
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/coderrupee`.
...
Until enough shared service code lives outside `packages/coderrupee`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/coderrupee`

It should not import `packages/coderrupee` services while `packages/coderrupee`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on coderrupee-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/coderrupee` implementation details.
