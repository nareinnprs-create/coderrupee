...
Trim `packages/coderrupee/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/coderrupee/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/coderrupee`.
- `[x]` `bun typecheck` from `packages/coderrupee`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/coderrupee/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/coderrupee`.
- `[x]` `bun dev generate > /tmp/coderrupee-openapi.json` from `packages/coderrupee`.
...
- Edit `packages/coderrupee/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/coderrupee-openapi.json` from `packages/coderrupee`.
...
- Check source schemas in files like `packages/coderrupee/src/session/schema.ts`, `packages/coderrupee/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/coderrupee/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/coderrupee/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
