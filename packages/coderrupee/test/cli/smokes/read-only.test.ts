...
describe("coderrupee read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // CODERRUPEE_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ coderrupee }) =>
      Effect.gen(function* () {
        const r = yield* coderrupee.spawn(["mcp", "list"])
        coderrupee.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ coderrupee }) =>
      Effect.gen(function* () {
        const r = yield* coderrupee.spawn(["providers", "list"])
        coderrupee.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ coderrupee }) =>
      Effect.gen(function* () {
        const r = yield* coderrupee.spawn(["models"])
        coderrupee.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ coderrupee }) =>
      Effect.gen(function* () {
        const r = yield* coderrupee.spawn(["agent", "list"])
        coderrupee.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh CODERRUPEE_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ coderrupee }) =>
      Effect.gen(function* () {
        const r = yield* coderrupee.spawn(["session", "list"])
        coderrupee.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ coderrupee }) =>
      Effect.gen(function* () {
        const r = yield* coderrupee.spawn(["stats"])
        coderrupee.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under CODERRUPEE_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ coderrupee }) =>
      Effect.gen(function* () {
        const r = yield* coderrupee.spawn(["db", "path"])
        coderrupee.expectExit(r, 0, "db path")
