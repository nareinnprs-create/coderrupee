...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ coderrupee }) =>
    Effect.gen(function* () {
      const result = yield* coderrupee.spawn(["--replay-limit", "10"])

      coderrupee.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ coderrupee }) =>
    Effect.gen(function* () {
      const result = yield* coderrupee.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ coderrupee }) =>
    Effect.gen(function* () {
      const result = yield* coderrupee.spawn(["--mini", "--port", "4096"])
