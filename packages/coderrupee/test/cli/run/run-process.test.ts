// Subprocess integration tests for `coderrupee run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `coderrupee.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `CODERRUPEE_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("coderrupee run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, coderrupee }) =>
      Effect.gen(function* () {
        ...
        const result = yield* coderrupee.run("say hi")
        coderrupee.expectExit(result, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, coderrupee }) =>
      Effect.gen(function* () {
        ...
        const result = yield* coderrupee.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, coderrupee }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* coderrupee.run("think", { extraArgs: ["--thinking"] })
        coderrupee.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* coderrupee.run("think again")
        coderrupee.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, coderrupee }) =>
      Effect.gen(function* () {
        ...
        const result = yield* coderrupee.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, coderrupee }) =>
      Effect.gen(function* () {
        ...
        const result = yield* coderrupee.run("say hi", { format: "json" })
        coderrupee.expectExit(result, 0)

        const events = coderrupee.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ coderrupee }) =>
      Effect.gen(function* () {
        const result = yield* coderrupee.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, coderrupee }) =>
      Effect.gen(function* () {
        ...
        const result = yield* coderrupee.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, coderrupee }) =>
      Effect.gen(function* () {
        ...
        yield* llm.text("recovered")
        const result = yield* coderrupee.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, coderrupee }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* coderrupee.run("request permission", { permission: { bash: "ask" } })
        coderrupee.expectExit(denied, 0)
        ...
        const allowed = yield* coderrupee.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        coderrupee.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* coderrupee.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        coderrupee.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, coderrupee }) =>
      Effect.gen(function* () {
        ...
        const server = yield* coderrupee.serve()

        const result = yield* coderrupee.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, coderrupee }) =>
      Effect.gen(function* () {
        const result = yield* coderrupee.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, coderrupee }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* coderrupee.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
