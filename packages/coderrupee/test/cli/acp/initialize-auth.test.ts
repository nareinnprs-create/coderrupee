...
describe("coderrupee acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ coderrupee }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ coderrupee }))
        ...
        expect(initialized.agentInfo?.name).toBe("CoderRupee")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ coderrupee }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ coderrupee })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("coderrupee-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "coderrupee-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.CODERRUPEE_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ coderrupee }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("coderrupee-login")
