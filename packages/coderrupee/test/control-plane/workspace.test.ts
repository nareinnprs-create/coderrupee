...
import { EventSequenceTable } from "@coderrupee/core/event/sql"
...
import { WorkspaceV2 } from "@coderrupee/core/workspace"
import { WorkspaceTable } from "@coderrupee/core/control-plane/workspace.sql"
...
import { Ripgrep } from "@coderrupee/core/ripgrep"
...
const originalEnv = {
  CODERRUPEE_AUTH_CONTENT: process.env.CODERRUPEE_AUTH_CONTENT,
  CODERRUPEE_EXPERIMENTAL_WORKSPACES: process.env.CODERRUPEE_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.CODERRUPEE_EXPERIMENTAL_WORKSPACES = "true"
})
...
async function initGitRepo(dir: string) {
  ...
  await $`git config user.email "test@coderrupee.test"`.cwd(dir).quiet()
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.CODERRUPEE_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        process.env.OTEL_EXPORTER_OTLP_HEADERS = "authorization=otel"
        process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://otel.test"
        process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=coderrupee-test"
        ...
        expect(JSON.parse(recorded.calls.create[0].env.CODERRUPEE_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.CODERRUPEE_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.CODERRUPEE_EXPERIMENTAL_WORKSPACES).toBe("true")
        ...
        expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=coderrupee-test")
