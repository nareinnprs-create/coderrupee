...
it.instance("plan agent denies edits except .coderrupee/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".coderrupee/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".coderrupee", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.CODERRUPEE_TEST_HOME
      process.env.CODERRUPEE_TEST_HOME = test.directory
