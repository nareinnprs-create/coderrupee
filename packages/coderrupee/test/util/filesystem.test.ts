...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "coderrupee.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "coderrupee.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "coderrupee.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "coderrupee.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["coderrupee.json", "coderrupee.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "coderrupee.json"),
        path.join(project, "coderrupee.jsonc"),
      ])
    })
  })
