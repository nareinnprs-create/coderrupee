...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.CODERRUPEE_TEST ?? "")'), {
      env: {
        CODERRUPEE_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "CODERRUPEE_TEST_SHELL"], {
      shell: true,
      env: {
        CODERRUPEE_TEST_SHELL: "ok",
      },
    })

    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("CODERRUPEE_TEST_SHELL=ok")
  })
