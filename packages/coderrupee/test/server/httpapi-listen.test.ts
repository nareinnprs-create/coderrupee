...
const original = {
  ...
  envPassword: process.env.CODERRUPEE_SERVER_PASSWORD,
  envUsername: process.env.CODERRUPEE_SERVER_USERNAME,
}
const auth = { username: "coderrupee", password: "listen-secret" }
const testPty = process.platform === "win32" ? test.skip : test

afterEach(async () => {
  Flag.CODERRUPEE_SERVER_PASSWORD = original.CODERRUPEE_SERVER_PASSWORD
  Flag.CODERRUPEE_SERVER_USERNAME = original.CODERRUPEE_SERVER_USERNAME
  if (original.envPassword === undefined) delete process.env.CODERRUPEE_SERVER_PASSWORD
  else process.env.CODERRUPEE_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) delete process.env.CODERRUPEE_SERVER_USERNAME
  else process.env.CODERRUPEE_SERVER_USERNAME = original.envUsername
  await disposeAllInstances()
  await resetDatabase()
})

async function startListener() {
  Flag.CODERRUPEE_SERVER_PASSWORD = auth.password
  Flag.CODERRUPEE_SERVER_USERNAME = auth.username
  process.env.CODERRUPEE_SERVER_PASSWORD = auth.password
  process.env.CODERRUPEE_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}

async function startNoAuthListener() {
  Flag.CODERRUPEE_SERVER_PASSWORD = undefined
  Flag.CODERRUPEE_SERVER_USERNAME = auth.username
  delete process.env.CODERRUPEE_SERVER_PASSWORD
  process.env.CODERRUPEE_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}
...
async function requestTicket(
  ...
  const response = await fetch(new URL(PtyPaths.connectToken.replace(":ptyID", id), listener.url), {
    ...
    headers: {
      ...
      "x-coderrupee-directory": dir,
      ...(options?.ticketHeader === false ? {} : { "x-coderrupee-ticket": "1" }),
...
describe("HttpApi Server.listen", () => {
  testPty("serves HTTP routes and upgrades PTY websocket through Server.listen", async () => {
    ...
    try {
      const response = await fetch(new URL(PtyPaths.shells, listener.url), {
        headers: { authorization: authorization(), "x-coderrupee-directory": tmp.path },
      })
      expect(response.status).toBe(200)
  ...
  test("plugin client requests reuse the listening server instance", async () => {
    ...
    const previous = process.env.CODERRUPEE_DISABLE_DEFAULT_PLUGINS
    process.env.CODERRUPEE_DISABLE_DEFAULT_PLUGINS = "1"
    ...
    try {
      ...
      if (previous === undefined) delete process.env.CODERRUPEE_DISABLE_DEFAULT_PLUGINS
      else process.env.CODERRUPEE_DISABLE_DEFAULT_PLUGINS = previous
    }
  })
  ...
  testPty("rejects unsafe PTY ticket mint and connect requests", async () => {
    ...
    try {
      ...
      const ambiguous = await fetch(new URL(PtyPaths.connectToken.replace(":ptyID", info.id), listener.url), {
        method: "POST",
        headers: { authorization: authorization(), "x-coderrupee-ticket": "1" },
      })
