// Subprocess test harness for the coderrupee CLI. Spawns the real binary against
...
//
// Configuration flows through coderrupee's built-in test affordances:
//   - CODERRUPEE_CONFIG_CONTENT      : provider config inline, no files to find
//   - CODERRUPEE_TEST_HOME           : pins os.homedir() → tmpdir
//   - CODERRUPEE_DISABLE_PROJECT_CONFIG : skip walking up for coderrupee.json
//   - CODERRUPEE_PURE                : skip external plugin discovery + install
//   - CODERRUPEE_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `coderrupee.run` is fully wired. The shape supports adding more
// builders (`coderrupee.serve(opts)`, `coderrupee.acp(opts)`, `coderrupee.auth(...)`)
...
import { AppProcess } from "@coderrupee/core/process"
...
const coderrupeeRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(coderrupeeRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    CODERRUPEE_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    CODERRUPEE_CONFIG_CONTENT: configJson,
    CODERRUPEE_DISABLE_PROJECT_CONFIG: "1",
    CODERRUPEE_PURE: "1",
    CODERRUPEE_DISABLE_AUTOUPDATE: "1",
    CODERRUPEE_DISABLE_AUTOCOMPACT: "1",
    CODERRUPEE_DISABLE_MODELS_FETCH: "1",
    CODERRUPEE_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `coderrupee run`. New flags should
...
// `coderrupee serve` is a long-lived process — it never exits on its own.
...
// `coderrupee acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type OpencodeCli = {
  ...
  // Spawn `coderrupee serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `coderrupee acp` and return a duplex JSON-RPC handle. Long-lived:
...
export type CliFixture = {
  ...
  readonly coderrupee: OpencodeCli
}
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("coderrupee.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          CODERRUPEE_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("coderrupee.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("coderrupee.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "coderrupee server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `coderrupee serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("coderrupee.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const coderrupee: OpencodeCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, coderrupee })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `coderrupee.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "coderrupee") {
...
// every call site. Use this for any test that needs the coderrupee CLI fixture.
//
...
// resources (e.g. `coderrupee.serve`) without an extra `Effect.scoped` wrapper —
