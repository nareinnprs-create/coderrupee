    ...
    const msgs = [
      ...
      { role: "assistant", content: text("assistant string") },
      {
        ...
        content: [
          ...
          { type: "tool-call", toolCallId: "call-1", toolName: "Read", input: { filePath: ".coderrupee/tool/emoji.ts" } },
          {
            type: "tool-result",
            toolCallId: "call-2",
            toolName: "Read",
...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    const coderrupeeModel = {
      ...openaiModel,
      providerID: "coderrupee",
      api: {
        id: "coderrupee-test",
        url: "https://api.coderrupee.ai",
        npm: "@ai-sdk/openai-compatible",
      },
    }
    const msgs = [
    ...
    const result = ProviderTransform.message(msgs, coderrupeeModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.coderrupee?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.coderrupee?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    const msgs = [
      {
        ...
        providerOptions: {
          ...
          coderrupee: { itemId: "msg_coderrupee" },
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              coderrupee: { itemId: "msg_coderrupee_part" },
    ...
    expect(result[0].providerOptions?.coderrupee?.itemId).toBe("msg_coderrupee")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.coderrupee?.itemId).toBe("msg_coderrupee_part")
...
describe("ProviderTransform sampling defaults - DeepSeek", () => {
  ...
  test.each([
    ...
    ["coderrupee", "deepseek-v4-flash"],
    ["coderrupee-go", "deepseek-v4-flash"],
