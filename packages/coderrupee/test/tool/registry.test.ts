...
const configLayer = TestConfig.layer({
  directories: () => InstanceState.directory.pipe(Effect.map((dir) => [path.join(dir, ".coderrupee")])),
})
...
describe("tool.registry", () => {
  ...
  it.instance("loads tools from .coderrupee/tool (singular)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const coderrupee = path.join(test.directory, ".coderrupee")
      const tool = path.join(coderrupee, "tool")
  ...
  it.instance("ignores non-tool exports in .coderrupee/tool files", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tool = path.join(test.directory, ".coderrupee", "tool")
      yield* Effect.promise(() => fs.mkdir(tool, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  // came in through `plugin.list()` — `oh-my-coderrupee` was registering a tool
  ...
  it.instance("loads tools from .coderrupee/tools (plural)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const coderrupee = path.join(test.directory, ".coderrupee")
      const tools = path.join(coderrupee, "tools")
  ...
  it.instance("loads Zod-schema custom tools with JSON Schema and validation", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const customTools = path.join(test.directory, ".coderrupee", "tools")
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const coderrupee = path.join(test.directory, ".coderrupee")
        const customTools = path.join(coderrupee, "tools")
        const plugin = path.join(coderrupee, "node_modules", "@coderrupee-ai", "plugin")
        yield* Effect.promise(() => fs.mkdir(path.join(plugin, "dist"), { recursive: true }))
        yield* Effect.promise(() => fs.mkdir(customTools, { recursive: true }))
        yield* Effect.promise(() =>
          fs.cp(path.dirname(fileURLToPath(import.meta.resolve("zod"))), path.join(coderrupee, "node_modules", "zod"), {
            dereference: true,
            recursive: true,
          }),
        )
        yield* Effect.promise(() =>
          Bun.write(
            ...
            JSON.stringify({ name: "@coderrupee/plugin", type: "module", exports: { ".": "./dist/index.js" } }),
          ),
        )
        yield* Effect.promise(() =>
          Bun.write(
        ...
        yield* Effect.promise(() =>
          Bun.write(
            ...
            [
              'import { tool } from "@coderrupee/plugin"',
              "export default tool({",
  ...
  it.instance("loads legacy JSON-schema-shaped custom tools with wire schema", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tools = path.join(test.directory, ".coderrupee", "tools")
      yield* Effect.promise(() => fs.mkdir(tools, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  it.instance("loads tools with external dependencies without crashing", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(coderrupee, "package.json"),
          JSON.stringify({
            name: "custom-tools",
            dependencies: {
              "@coderrupee/plugin": "^0.0.0",
              cowsay: "^1.6.0",
            },
          }),
        ),
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(coderrupee, "package-lock.json"),
          JSON.stringify({
            name: "custom-tools",
            lockfileVersion: 3,
            packages: {
      ...
      const cowsay = path.join(coderrupee, "node_modules", "cowsay")
