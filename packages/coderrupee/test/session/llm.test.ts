...
import { LLMClient, RequestExecutor } from "@coderrupee/llm/route"
...
describe("session.llm.stream", () => {
  ...
  const coderrupeeFixture = { providerID: "coderrupee-test", modelID: vivgridFixture.modelID }

  it.instance(
    "sends the parent session header for coderrupee providers",
    () =>
      Effect.gen(function* () {
        ...
        const resolved = yield* Provider.use.getModel(
          ProviderV2.ID.make(coderrupeeFixture.providerID),
          ModelV2.ID.make(coderrupeeFixture.modelID),
        )
        ...
        const user = {
          ...
          agent: agent.name,
          model: {
            providerID: ProviderV2.ID.make(coderrupeeFixture.providerID),
            modelID: resolved.id,
          },
      ...
      }),
    {
      config: () => {
        const fixture = loadFixture(vivgridFixture.providerID, vivgridFixture.modelID)
        return {
          enabled_providers: [coderrupeeFixture.providerID],
          provider: {
            [coderrupeeFixture.providerID]: {
              name: "CoderRupee Test",
