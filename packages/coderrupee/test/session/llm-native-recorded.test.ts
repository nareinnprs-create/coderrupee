...
import { SessionV1 } from "@coderrupee/core/v1/session"
import { ModelsDev } from "@coderrupee/core/models-dev"
import { HttpRecorder } from "@coderrupee/http-recorder"
import { HttpRecorderInternal } from "@coderrupee/http-recorder/internal"
...
import { LLMEvent, LLMResponse } from "@coderrupee/llm"
import { RequestExecutor } from "@coderrupee/llm/route"
...
const zenURL = (connection: string) => `https://console.coderrupee.ai/proxy/connections/${connection}/v1`
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.CODERRUPEE_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["coderrupee", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("CODERRUPEE_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("CODERRUPEE_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["coderrupee", "native", "oauth", "tool-loop"],
  ...
  {
    id: "coderrupee-proxy",
    name: "CoderRupee proxy",
    providerID: ProviderV2.ID.coderrupee,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["coderrupee", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.CODERRUPEE_RECORD_CONSOLE_TOKEN && process.env.CODERRUPEE_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.coderrupee,
        name: "CoderRupee Zen",
        env: ["CODERRUPEE_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.CODERRUPEE_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.CODERRUPEE_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.CODERRUPEE_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["coderrupee", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("CODERRUPEE_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("CODERRUPEE_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("CODERRUPEE_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set CODERRUPEE_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the coderrupee LLM stack remain real.
  const metadata = {
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "coderrupee.json"),
      JSON.stringify({ $schema: "https://coderrupee.ai/config.json", ...scenario.config(model) }),
    ),
  )
