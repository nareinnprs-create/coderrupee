...
import { LLMEvent, ToolFailure } from "@coderrupee/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor, type LLMClientShape } from "@coderrupee/llm/route"
...
describe("session.llm-native.request", () => {
  ...
  test("selects native request routes for provider packages", () => {
    ...
    const compatible = LLMNative.model({
      model: {
        ...baseModel,
        providerID: ProviderV2.ID.make("coderrupee"),
  ...
  test("only enables native runtime for supported OpenAI API-key models", () => {
    ...
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("coderrupee") },
        provider: { ...providerInfo, id: ProviderV2.ID.make("coderrupee") },
        auth: undefined,
      }),
    ).toMatchObject({
      type: "supported",
    ...
    ).toEqual({ type: "unsupported", reason: "provider is not openai, coderrupee, or anthropic" })
    expect(
      LLMNativeRuntime.status({
        model: baseModel,
  ...
  test("prefers console provider api key over stored coderrupee auth", () => {
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("coderrupee") },
        provider: {
          ...providerInfo,
          id: ProviderV2.ID.make("coderrupee"),
