import type { PermissionV1 } from "@coderrupee/core/v1/permission"
import { FSUtil } from "@coderrupee/core/fs-util"
// CLI entry point for `coderrupee run` and `coderrupee --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`coderrupee --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`coderrupee --mini --attach`): connects to a running
//      coderrupee server and runs interactive mode against it.
//
...
import { createOpencodeClient, type OpencodeClient, type ToolPart } from "@coderrupee/sdk/v2"
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run coderrupee with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running coderrupee server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to CODERRUPEE_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to CODERRUPEE_SERVER_USERNAME or 'coderrupee')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      const sdk = createOpencodeClient({
        baseUrl: "http://coderrupee.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
