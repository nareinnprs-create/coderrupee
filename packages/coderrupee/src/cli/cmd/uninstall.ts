...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall coderrupee and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall CoderRupee")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g coderrupee-ai",
      pnpm: "pnpm uninstall -g coderrupee-ai",
      bun: "bun remove -g coderrupee-ai",
      yarn: "yarn global remove coderrupee-ai",
      brew: "brew uninstall coderrupee",
      choco: "choco uninstall coderrupee",
      scoop: "scoop uninstall coderrupee",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "coderrupee", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using CoderRupee!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# coderrupee") || content.includes(".coderrupee/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".coderrupee/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".coderrupee/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".coderrupee"))
    ) {
      continue
    }
