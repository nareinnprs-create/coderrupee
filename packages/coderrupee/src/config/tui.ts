...
import { isRecord } from "@coderrupee/tui/util/record"
...
import { TuiKeybind } from "@coderrupee/tui/config/keybind"
...
export class Service extends Context.Service<Service, Interface>()("@coderrupee/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old coderrupee.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.coderrupee`
  // folders between cwd and home, and CODERRUPEE_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.CODERRUPEE_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit CODERRUPEE_TUI_CONFIG override, if set.
  if (Flag.CODERRUPEE_TUI_CONFIG) {
    const configFile = Flag.CODERRUPEE_TUI_CONFIG
  ...
  // 4. `.coderrupee` directories (and CODERRUPEE_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".coderrupee") || dir === Flag.CODERRUPEE_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".coderrupee") && dir !== Flag.CODERRUPEE_CONFIG_DIR) continue
