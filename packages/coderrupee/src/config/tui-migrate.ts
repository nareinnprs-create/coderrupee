...
const TUI_SCHEMA_URL = "https://coderrupee.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from coderrupee.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const coderrupee = await coderrupeeFiles(input)
  for (const file of coderrupee) {
...
async function coderrupeeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "coderrupee"),
    ...(await Filesystem.findUp(["coderrupee.json", "coderrupee.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "coderrupee"))
  }
  if (Flag.CODERRUPEE_CONFIG) files.push(Flag.CODERRUPEE_CONFIG)
