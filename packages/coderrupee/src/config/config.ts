...
import { serviceUse } from "@coderrupee/core/effect/service-use"
...
import { InstallationLocal, InstallationVersion } from "@coderrupee/core/installation/version"
import { existsSync } from "fs"
import { Account } from "@/account/account"
import { isRecord } from "@/util/record"
import type { ConsoleState } from "@coderrupee/core/v1/config/console-state"
import { FSUtil } from "@coderrupee/core/fs-util"
import { InstanceState } from "@/effect/instance-state"
import { Context, Duration, Effect, Exit, Fiber, Layer, Option, Schema } from "effect"
import { FetchHttpClient, HttpClient, HttpClientRequest } from "effect/unstable/http"
import { EffectFlock } from "@coderrupee/core/util/effect-flock"
import { containsPath, type InstanceContext } from "../project/instance-context"
import { ConfigV1 } from "@coderrupee/core/v1/config/config"
import { RemoteAuthError } from "@coderrupee/core/v1/config/error"
import { ConfigPermissionV1 } from "@coderrupee/core/v1/config/permission"
...
export class Service extends Context.Service<Service, Interface>()("@coderrupee/Config") {}

export const use = serviceUse(Service)

function globalConfigFile() {
  const candidates = ["coderrupee.jsonc", "coderrupee.json", "config.json"].map((file) =>
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://coderrupee.ai/config.json"
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.CODERRUPEE_CONFIG && !Flag.CODERRUPEE_CONFIG_DIR && !Flag.CODERRUPEE_CONFIG_CONTENT) {
        const file = globalConfigFile()
        if (!existsSync(file)) {
          yield* fs
            .writeWithDirs(file, JSON.stringify({ $schema: "https://coderrupee.ai/config.json" }, null, 2))
            .pipe(Effect.catch(() => Effect.void))
        }
      }
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "config.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "coderrupee.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "coderrupee.jsonc"), env))
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://coderrupee.ai/config.json"
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "CODERRUPEE_CONFIG_CONTENT") return "local"
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            const wellknownURL = `${url}/.well-known/coderrupee`
            ...
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://coderrupee.ai/config.json"
        ...
        if (Flag.CODERRUPEE_CONFIG) {
          yield* merge(Flag.CODERRUPEE_CONFIG, yield* loadFile(Flag.CODERRUPEE_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.CODERRUPEE_CONFIG })
        }

        if (!Flag.CODERRUPEE_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("coderrupee", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.CODERRUPEE_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from CODERRUPEE_CONFIG_DIR", { path: Flag.CODERRUPEE_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".coderrupee") || dir === Flag.CODERRUPEE_CONFIG_DIR) {
            for (const file of ["coderrupee.json", "coderrupee.jsonc"]) {
          ...
          // Auto-discovered plugins under `.coderrupee/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.CODERRUPEE_CONFIG_CONTENT) {
          const source = "CODERRUPEE_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.CODERRUPEE_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          yield* Effect.logDebug("loaded custom config from CODERRUPEE_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["CODERRUPEE_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("CODERRUPEE_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.CODERRUPEE_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.CODERRUPEE_PERMISSION))
          } catch (err) {
            yield* Effect.logWarning("CODERRUPEE_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.CODERRUPEE_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.CODERRUPEE_DISABLE_PRUNE) {
