...
import { NonNegativeInt, PositiveInt } from "@coderrupee/core/schema"
import { ConfigAttachmentV1 } from "@coderrupee/core/v1/config/attachment"
import { ConfigLSPV1 } from "@coderrupee/core/v1/config/lsp"
import { InvalidError } from "@coderrupee/core/v1/config/error"
  ...
  if (permissions.length)
    throw new InvalidError({
      ...
      issues: permissions.map((path) => ({
        path,
        message: 'V2 permissions are not supported by CoderRupee V1. Use V1 "permission" rules or run coderrupee2.',
      })),
    })
