...
export type PatchDeps = {
  ...
  files: (dir: string, name: "coderrupee" | "tui") => string[]
}
...
function patchDir(input: PatchInput) {
  ...
  return path.join(root, ".coderrupee")
}

function patchName(kind: Kind): "coderrupee" | "tui" {
  if (kind === "server") return "coderrupee"
  return "tui"
}
