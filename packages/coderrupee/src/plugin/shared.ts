...
export const DEPRECATED_PLUGIN_PACKAGES = ["coderrupee-openai-codex-auth", "coderrupee-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, coderrupeeVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(coderrupeeVersion) || semver.major(coderrupeeVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.coderrupee
  if (typeof range !== "string") return
  if (!semver.satisfies(coderrupeeVersion, range)) {
    throw new Error(`Plugin requires coderrupee ${range} but running ${coderrupeeVersion}`)
  }
}
