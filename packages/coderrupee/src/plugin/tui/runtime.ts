...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@coderrupee/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@coderrupee/core/effect/app-node-builder"
import { errorData, errorMessage } from "@coderrupee/tui/util/error"
...
import { hasTheme, upsertTheme } from "@coderrupee/tui/context/theme"
import { Global } from "@coderrupee/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@coderrupee/core/util/flock"
import { Flag } from "@coderrupee/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@coderrupee/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@coderrupee/core/v1/config/plugin"
import { createCommandShim } from "@coderrupee/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@coderrupee/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".coderrupee"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".coderrupee", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".coderrupee", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.CODERRUPEE_PURE ? [] : pluginOrigins
    if (Flag.CODERRUPEE_PURE && pluginOrigins.length) {
    }
