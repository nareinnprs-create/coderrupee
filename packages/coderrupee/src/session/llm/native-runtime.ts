...
import type { LLMClientShape } from "@coderrupee/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("coderrupee"))
    return { type: "unsupported", reason: "provider is not openai, coderrupee, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @coderrupee/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains coderrupee-owned. The native runtime only adapts
      // the @coderrupee/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
