...
import type { AssistantMessage, Message, OpencodeClient, SessionMessageResponse } from "@coderrupee/sdk/v2"
...
export const AuthMethodID = "coderrupee-login"
...
export class Service extends Context.Service<Service, Interface>()("@coderrupee/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `coderrupee auth login` in the terminal",
      name: "Login with coderrupee",
      id: AuthMethodID,
    }
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, coderrupee provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const coderrupeeProvider = providers[ProviderV2.ID.make("coderrupee")]
  const coderrupeeModel = coderrupeeProvider ? Provider.sort(Object.values(coderrupeeProvider.models))[0] : undefined
  if (coderrupeeProvider && coderrupeeModel) return { providerID: coderrupeeProvider.id, modelID: coderrupeeModel.id }
...
function promptErrorMessage(error: AssistantError) {
  ...
  return "CoderRupee prompt failed"
}
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "CoderRupee service failure", service })
}
