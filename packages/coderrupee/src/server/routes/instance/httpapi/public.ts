...
import { CoderRupeeHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for coderrupee serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(CoderRupeeHttpApi)
...
export const PublicApi = CoderRupeeHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "coderrupee",
    version: "1.0.0",
    description: "coderrupee api",
