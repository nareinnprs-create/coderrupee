...
import { PromptInput } from "@coderrupee/schema/prompt-input"
import { Session } from "@coderrupee/schema/session"
import { Project } from "@coderrupee/schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@coderrupee/schema/schema"
...
import { SessionEvent } from "@coderrupee/schema/session-event"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this CoderRupee process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this CoderRupee process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(
