...
const reg = process.env.REGISTRY ?? "ghcr.io/nareinnprs-create"
...
const setup = async () => {
  ...
  if (list.includes("coderrupee")) {
    await $`docker buildx use coderrupee`
    return
  }
  await $`docker buildx create --name coderrupee --use`
}
