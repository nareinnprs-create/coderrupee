...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@coderrupee/acme")).toBe("@coderrupee/acme")
    expect(Npm.sanitize("@coderrupee/acme@1.0.0")).toBe("@coderrupee/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/coderrupee/acme.git"
    const expected = win ? "acme@git+https_//github.com/coderrupee/acme.git" : spec
