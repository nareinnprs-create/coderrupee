...
import { ModelsDevPlugin } from "@coderrupee/core/plugin/models-dev"
...
describe("ModelsDevPlugin", () => {
  ...
  it.effect("registers key methods for providers with environment variables", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          path: Flag.CODERRUPEE_MODELS_PATH,
          disabled: Flag.CODERRUPEE_DISABLE_MODELS_FETCH,
        }
        Flag.CODERRUPEE_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
        Flag.CODERRUPEE_DISABLE_MODELS_FETCH = true
        return previous
      }),
      () =>
        Effect.gen(function* () {
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.CODERRUPEE_MODELS_PATH = previous.path
          Flag.CODERRUPEE_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})
