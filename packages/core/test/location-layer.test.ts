...
import { define } from "@coderrupee/plugin/v2/effect"
...
import { LocationServiceMap } from "@coderrupee/core/location-services"
      ...
      Effect.flatMap(([blocked, allowed]) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(blocked.path, "coderrupee.json"),
              JSON.stringify({
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((dir) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(dir.path, "coderrupee.json"),
              JSON.stringify({
                providers: {
                  unavailable: {
                    name: "Unavailable",
