...
import { Watcher } from "@coderrupee/core/filesystem/watcher"
...
const flagsLayer = ConfigProvider.layer(
  ConfigProvider.fromUnknown({
    CODERRUPEE_EXPERIMENTAL_FILEWATCHER: "true",
    CODERRUPEE_EXPERIMENTAL_DISABLE_FILEWATCHER: "false",
  }),
)
...
function withTmp<A, E, R>(
  ...
  return Effect.acquireRelease(
    Effect.promise(async () => {
      ...
      await $`git config user.email test@coderrupee.test`.cwd(tmp.path).quiet()
