...
const coderrupeeClient = process.env.CODERRUPEE_CLIENT

afterEach(() => {
  if (otelResourceAttributes === undefined) delete process.env.OTEL_RESOURCE_ATTRIBUTES
  else process.env.OTEL_RESOURCE_ATTRIBUTES = otelResourceAttributes

  if (coderrupeeClient === undefined) delete process.env.CODERRUPEE_CLIENT
  else process.env.CODERRUPEE_CLIENT = coderrupeeClient
})

describe("resource", () => {
  test("parses and decodes OTEL resource attributes", () => {
    process.env.OTEL_RESOURCE_ATTRIBUTES =
      "service.namespace=coderrupee,team=platform%2Cobservability,label=hello%3Dworld,key%2Fname=value%20here"

    expect(resource().attributes).toMatchObject({
      "service.namespace": "coderrupee",
  ...
  test("drops OTEL resource attributes when any entry is invalid", () => {
    process.env.OTEL_RESOURCE_ATTRIBUTES = "service.namespace=coderrupee,broken"

    expect(resource().attributes["service.namespace"]).toBeUndefined()
    expect(resource().attributes["coderrupee.client"]).toBeDefined()
  })

  test("keeps built-in attributes when env values conflict", () => {
    process.env.CODERRUPEE_CLIENT = "cli"
    process.env.OTEL_RESOURCE_ATTRIBUTES =
      "coderrupee.client=web,service.instance.id=override,service.namespace=coderrupee"

    expect(resource().attributes).toMatchObject({
      "coderrupee.client": "cli",
      "service.namespace": "coderrupee",
    })
    expect(resource().attributes["service.instance.id"]).not.toBe("override")
    expect(resource().attributes["coderrupee.run"]).toMatch(/^[0-9a-f]{8}$/)
  })
})

test("file logger appends concurrent runs with a run on every line", async () => {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "coderrupee-log-test-"))
  await using _ = {
    async [Symbol.asyncDispose]() {
      await fs.rm(dir, { recursive: true, force: true })
    },
  }
  const file = path.join(dir, "coderrupee.log")
