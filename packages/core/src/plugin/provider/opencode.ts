...
import type { IntegrationOAuthMethodRegistration } from "@coderrupee/plugin/v2/effect/integration"
import { define } from "@coderrupee/plugin/v2/effect/plugin"
import type { CredentialValue } from "@coderrupee/sdk/v2/types"
...
const defaultServer = "https://coderrupee.ai/console"
const clientID = "coderrupee-cli"
...
export const OpencodePlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  ...
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("OpencodePlugin.load")(function* () {
      const connection = yield* ctx.integration.connection.active("coderrupee")
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load CoderRupee provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
    ...
    yield* ctx.integration.transform((draft) => {
      draft.update("coderrupee", (integration) => {
        integration.name = "CoderRupee"
      })
      draft.method.update(oauth(http))
      draft.method.update({ integrationID: "coderrupee", method: { type: "key", label: "API key (service account)" } })
    })

    connected = (yield* ctx.integration.connection.active("coderrupee")) !== undefined
    yield* ctx.catalog.transform((catalog) => {
      for (const [providerID, item] of Object.entries(providers ?? {})) {
        catalog.provider.update(providerID, (provider) => {
          provider.integrationID = Integration.ID.make("coderrupee")
      ...
      const item = catalog.provider.get(ProviderV2.ID.coderrupee)
      if (!item) return
      const hasKey = Boolean(process.env.CODERRUPEE_API_KEY || connected || item.provider.request.body.apiKey)
    ...
    yield* events.subscribe(Integration.Event.ConnectionUpdated).pipe(
      Stream.filter((event) => event.data.integrationID === Integration.ID.make("coderrupee")),
