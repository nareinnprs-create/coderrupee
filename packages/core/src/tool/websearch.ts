...
import { ToolFailure } from "@coderrupee/llm"
...
export class ConfigService extends Context.Service<ConfigService, Config>()("@coderrupee/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.CODERRUPEE_WEBSEARCH_PROVIDER === "exa" || process.env.CODERRUPEE_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.CODERRUPEE_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("CODERRUPEE_EXPERIMENTAL") || truthy("CODERRUPEE_ENABLE_EXA") || truthy("CODERRUPEE_EXPERIMENTAL_EXA"),
    enableParallel: truthy("CODERRUPEE_ENABLE_PARALLEL") || truthy("CODERRUPEE_EXPERIMENTAL_PARALLEL"),
              ...
              const text =
                provider === "exa"
                  ...
                  : yield* callMcp(
                      ...
                      {
                        "User-Agent": `coderrupee/${InstallationVersion}`,
