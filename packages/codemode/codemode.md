...
This is the living design and status document for `@coderrupee/codemode` and its existing V2 CoderRupee adapter.
...
### V2 CoderRupee adapter

This section describes the `v2` branch integration. On `dev`, CodeMode is integrated through
`packages/coderrupee/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
